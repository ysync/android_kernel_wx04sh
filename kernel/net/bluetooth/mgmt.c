...
static int start_discovery(struct sock *sk, u16 index)
{
	...
	if (lmp_le_capable(hdev)) {
		...
		le_cp.interval = cpu_to_le16(0x0030);
		le_cp.window = cpu_to_le16(0x0030);
					...
					bdaddr_t *bdaddr, __le32 value)
{
	...
	if (!loc_mitm && !rem_mitm) {
		ev.auto_confirm = 1;
	} else if (conn->auth_initiator && (rem_cap == 0x00 || rem_cap == 0x03)) {
		ev.auto_confirm = 1;
	} else if (rem_cap == 0x00 || rem_cap == 0x03) {
		ev.just_works = 1;
	}
#else /* BLUETOOTH_CUSTOMIZE */
