...
static int phs_gpio_config(int flg)
{
	if (flg) {
		gpio_request(22, "PHS_TX");
		gpio_request(23, "PHS_RX");
		gpio_request(24, "PHS_CTS");
		gpio_request(25, "PHS_RFR");
	} else {
		gpio_free(22);
		gpio_free(23);
		gpio_free(24);
...
static struct msm_serial_hs_platform_data uart_gsbi5_pdata = {
	.wakeup_irq = 0,
	.gpio_config = phs_gpio_config,
	.tx_buf_size = 64,
};
...
struct platform_device msm8960_device_uart_gsbi5 = {
	...
	.dev	= {
		...
		.coherent_dma_mask	= DMA_BIT_MASK(32),
		.platform_data = &uart_gsbi5_pdata,
	},
};
...
static struct resource resouces_phs_mdm[] = {
	{
		.start	= MSM_UART5DM_PHYS,
		.end	= MSM_UART5DM_PHYS + PAGE_SIZE - 1,
		.name	= "phsmdm_addr",
...
struct platform_device device_phs_mdm = {
	.name			= "phs_mdm",
	.id				= 1,
	.num_resources	= ARRAY_SIZE(resouces_phs_mdm),
	.resource		= resouces_phs_mdm,
};
