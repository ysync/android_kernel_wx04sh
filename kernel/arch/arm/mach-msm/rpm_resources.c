...
#define RPMRS_SET_VDD_MIN_LOW (8)
#endif
...
#ifdef CONFIG_SHSYS_CUST
		if (!from_idle) {
			msm_rpmrs_resource_attr_show(NULL, &(msm_rpmrs_rpm_ctl.ko_attr), buf);
			sscanf(buf, "%u", &reqval);
			reqval |= (1 << RPMRS_SET_VDD_MIN_LOW);
			snprintf(buf, sizeof(buf), "%u", reqval);
			msm_rpmrs_resource_attr_store(NULL, &(msm_rpmrs_rpm_ctl.ko_attr), buf, sizeof(buf));
		}
#endif
