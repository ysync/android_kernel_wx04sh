...
void pil_put(void *peripheral_handle)
{
	...
	if (!strcmp(pil->desc->name, "modem")
	 || !strcmp(pil->desc->name, "modem_fw")
	 || !strcmp(pil->desc->name, "q6")) {
		if (!--pil->count) {
			printk("pil_put(%s): pil->count == 0 Invalid (from pid %d name %s)\n", pil->desc->name, current->pid, current->comm);
			WARN_ON(1);
			pil->count++;
		}
	}else{
