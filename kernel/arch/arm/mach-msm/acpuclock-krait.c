...
{
#ifdef CONFIG_SHSYS_CUST_PVS
	return 1;
#else /* CONFIG_SHSYS_CUST_PVS */
...
static int __init select_freq_plan(u32 pte_efuse_phys)
{
...
#ifdef CONFIG_SHSYS_CUST
	u32 pte_efuse_val, pvs, tbl_idx = 0;
	...
	if (pte_efuse) {
		...
		case 0x1:
			...
			if (krait_needs_vmin()) {
				...
				if (sh_smem_get_pvs_flg()) {
					pr_info("ACPU PVS: Nominal, but using Slow anyway\n");
					tbl_idx = PVS_SLOW;
				}else{
					pr_info("ACPU PVS: Nominal with no boost\n");
					tbl_idx = PVS_NOMINAL;
				}
			}else{
				tbl_idx = PVS_NOMINAL;
		...
		case 0x3:
			...
			if (krait_needs_vmin()) {
				enable_boost = false;
				pr_info("ACPU PVS: Fast, but using Nominal anyway\n");
				tbl_idx = PVS_NOMINAL;
			}else{
				tbl_idx = PVS_FAST;
			}
