...
#define TABLA_MBHC_FAKE_INSERT_HIGH 228
#else
...
/* SH_AUDIO_DRIVER -> *//*02-203*/
	SOC_SINGLE("RX1 Digital Mute", TABLA_A_CDC_RX1_B6_CTL, 0, 1, 0),
	SOC_SINGLE("RX2 Digital Mute", TABLA_A_CDC_RX2_B6_CTL, 0, 1, 0),
	SOC_SINGLE("RX5 Digital Mute", TABLA_A_CDC_RX5_B6_CTL, 0, 1, 0),
/* SH_AUDIO_DRIVER <- *//*02-203*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-097*/
/* Run-time support MicBias PCMLP XXXX0 */
		if (
		 ((tabla_codec_get_bias_mode() & TABLA_MICBIAS_MSK) == TABLA_MICBIAS_OFF)
		&& ((tabla_codec_get_bias_mode() & TABLA_DEBUGMODE_MSK) == TABLA_DEBUGMODE_ON)
		 ) {
			if ( w->reg == TABLA_A_MICB_1_CTL) {
				snd_soc_update_bits(codec, w->reg, 0x80, 0x00);
			} else if ( w->reg == TABLA_A_MICB_2_CTL) {
				snd_soc_update_bits(codec, w->reg, 0x80, 0x00);
			} else if ( w->reg == TABLA_A_MICB_3_CTL) {
				snd_soc_update_bits(codec, w->reg, 0x80, 0x00);
			}
		}
#endif /* CONFIG_SH_AUDIO_DRIVER *//*02-097*/
		if (tabla->mbhc_polling_active &&
...
tabla_codec_get_plug_type(struct snd_soc_codec *codec, bool highhph)
{
	...
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(codec);
/* SH_AUDIO_DRIVER -> *//*02-215*/
//	int num_det = MBHC_NUM_DCE_PLUG_DETECT + 1;
	int num_det = MBHC_NUM_DCE_PLUG_DETECT + 10;
/* SH_AUDIO_DRIVER -< *//*02-215*/
