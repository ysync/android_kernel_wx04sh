...
#ifdef CONFIG_BATTERY_SH
enum {
	DEBUG_SH_PM8xxx_SHOW_IRQ = BIT(0),
	DEBUG_SH_PM8xxx_SHOW_IRQ_ADC_EOC_USR_IRQ = BIT(1),
};
#define ADC_EOC_USR_IRQ 78
static int sh_pm8xxx_debug_mask;
module_param_named(
	debug_mask, sh_pm8xxx_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP
);
#endif
...
#ifdef CONFIG_BATTERY_SH
			if(sh_pm8xxx_debug_mask & DEBUG_SH_PM8xxx_SHOW_IRQ) {
				if((sh_pm8xxx_debug_mask & DEBUG_SH_PM8xxx_SHOW_IRQ_ADC_EOC_USR_IRQ)
				   || pmirq != ADC_EOC_USR_IRQ)
					pr_info("%d triggered (%d:%d)\n", irq, block, i);
			}
#endif
