...
#endif	/* CONFIG_SHIRDA */
#ifdef	CONFIG_PHS_MDM
#include <sharp/phs_mdm.h>
#endif	/* CONFIG_PHS_MDM */

/* #define	LOG_OUT */
/* #define	DEFAULT_LOG_OUT_ON	32 */
...
#define	LOGGING_PORT	"ttyHS1"	/* /dev/ttyHS1 */

#ifndef	NO_DUMP
static int dump(char *dir, const char *s, int length, int dumpsize)
{
	char line[BUF_SIZ];
	char asci[20];
	char tmp[10]; 
	...
	for (j = 0; j < len; j += 16) {
		sprintf(line, "%s%04x: ", dir, j);
		for (i = 0; i < 16; i++) {
			if (j + i < len) {
				sprintf(tmp, "%02x ", s[j + i]);
				strcat(line, tmp);
				asci[i] = (s[j + i] >= 0x20)&&(s[j + i] < 0x7f) ? s[j + i] : '.';
			} else {
				strcat(line, "   ");
				asci[i] = ' ';
			}
	...
	if (length - dumpsize > 0) {
		printk("%s%04x: more(%d bytes)...\n", dir, dumpsize, length - dumpsize);
	}
...
#ifdef	LOG_OUT
static int dumpsize_set(void *data, u64 val)
{
	struct msm_hs_port *msm_uport = data;

	msm_uport->dumpsize = val;
	return 0;
}

static int dumpsize_get(void *data, u64 *val)
{
	struct msm_hs_port *msm_uport = data;

	*val = msm_uport->dumpsize;
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(dumpsize_fops, dumpsize_get, dumpsize_set, "%llu\n");
#endif	/* LOG_OUT */
					   ...
					   int id)
{
	...
	if (IS_ERR_OR_NULL(msm_uport->loopback_dir))
		pr_err("%s(): Cannot create loopback.%d debug entry",
							__func__, id);
#ifdef	LOG_OUT
#ifdef	DEFAULT_LOG_OUT_ON
	msm_uport->dumpsize = DEFAULT_LOG_OUT_ON;
#endif
	snprintf(node_name, sizeof(node_name), "dumpsize.%d", id);
	msm_uport->dumpsize_dir = debugfs_create_file(node_name, S_IRUGO | S_IWUSR, debug_base, msm_uport, &dumpsize_fops);
	if (IS_ERR_OR_NULL(msm_uport->dumpsize_dir))
		pr_err("%s(): Cannot create dumpsize.%d debug entry", __func__, id);
#endif	/* LOG_OUT */
}
...
#ifdef	LOG_OUT
	debugfs_remove(msm_uport->dumpsize_dir);
#endif	/* LOG_OUT */
			       ...
			       unsigned int bps)
{
	...
	unsigned long data;
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
#if defined(CONFIG_SHIRDA) || defined(CONFIG_PHS_MDM)
	struct platform_device *pdev = to_platform_device(uport->dev);
#endif	/* CONFIG_SHIRDA || CONFIG_PHS_MDM */
...
#ifdef	LOG_OUT
		if (msm_uport->dumpsize) {
			struct tty_struct *tty = msm_uport->uport.state->port.tty;
			unsigned int rxfs;
			unsigned int txfs;

			printk("%s(): %s: dma_in_flight=%d, flush_receive=%d, tail=%d, head=%d, tx_ready_int_en=%d\n",
				__func__, tty->name,
				msm_uport->tx.dma_in_flight, msm_uport->tty_flush_receive, tx_buf->tail, tx_buf->head, tx->tx_ready_int_en);
			rxfs = msm_hs_read(uport, UARTDM_RXFS_ADDR);
			txfs = msm_hs_read(uport, UARTDM_TXFS_ADDR);

			if (txfs) {
				printk("%s: TXFS=0x%08x,Async=%d,Buffer=%d,State=%d, dma=%d\n", __func__, txfs,
					(txfs >> 10) & 0x0f, (txfs >> 7) & 0x07, (txfs & 0x7f) | (txfs >> 14), msm_uport->tx.dma_in_flight);
			}
			if (rxfs) {
				printk("%s: RXFS=0x%08x,Async=%d,Buffer=%d,State=%d, dma=%d\n", __func__, rxfs,
					(rxfs >> 10) & 0x0f, (rxfs >> 7) & 0x07, (rxfs & 0x7f) | (rxfs >> 14), msm_uport->tx.dma_in_flight);
			}
		}
#endif
...
static void msm_hs_submit_tx_locked(struct uart_port *uport)
{
...
#ifdef	CONFIG_PHS_MDM
	if (msm_uport->tx_buf_size < tx_count)
		tx_count = msm_uport->tx_buf_size;
#else	/* CONFIG_PHS_MDM */
	...
	if (msm_uport->dumpsize) {
		...
		printk("D>%s [%d bytes]\n", tty->name, tx_count);
#ifndef	NO_DUMP
		dump("D>", (const char *)&tx_buf->buf[tx_buf->tail], tx_count, msm_uport->dumpsize);
#endif	/* NO_DUMP */
	}
#endif	/* LOG_OUT */
...
static void msm_serial_hs_rx_tlet(unsigned long tlet_ptr)
{
	...
	if (msm_uport->dumpsize) {
		struct tty_struct *tty = msm_uport->uport.state->port.tty;
		printk("D<%s[%d bytes]\n", tty->name, rx_count);
		if (msm_uport->rx.buffer_pending & (FIFO_OVERRUN | PARITY_ERROR)) {
			printk("buffer_pending=%d, FIFO_OVERRUN=%d, PARITY_ERROR=%d\n",
				msm_uport->rx.buffer_pending,
				msm_uport->rx.buffer_pending & FIFO_OVERRUN ? 1 : 0,
				msm_uport->rx.buffer_pending & PARITY_ERROR ? 1 : 0);
		}
#ifndef	NO_DUMP
		dump("D<", msm_uport->rx.buffer, rx_count, msm_uport->dumpsize);
#endif	/* NO_DUMP */
	}
#endif	/* LOG_OUT */
...
void msm_hs_request_clock_on_internal(struct uart_port *uport);
...
static void msm_hs_start_tx_locked(struct uart_port *uport )
{
...
#ifdef	CONFIG_PHS_MDM
	if (msm_uport->purge_send) {
		struct msm_hs_tx *tx = &msm_uport->tx;
		struct circ_buf *tx_buf = &uport->state->xmit;
#ifdef	LOG_OUT
		if (msm_uport->dumpsize) {
			struct tty_struct *tty = msm_uport->uport.state->port.tty;
			printk("F>%s [%d bytes]\n", tty->name, tx->tx_count);
#ifndef	NO_DUMP
			dump("F>", (const char *)&tx_buf->buf[tx_buf->tail], tx->tx_count, 4096 /* msm_uport->dumpsize */);
#endif	/* NO_DUMP */
		}
#endif
		uport->icount.overrun += tx->tx_count;
	...
	if (msm_uport->tx.tx_ready_int_en == 0) {
		...
		if (msm_uport->tx.dma_in_flight == 0) {
			...
			if (msm_uport->dumpsize) {
				struct tty_struct *tty = msm_uport->uport.state->port.tty;
				const char *msm_hs_clk[] = {
					"PORT_OFF", "OFF", "REQUEST_OFF", "ON"
				};
				printk("%s: msm_hs_request_clock_on_internal() %s->MSM_HS_CLK_ON\n", tty->name, msm_hs_clk[msm_uport->clk_state]);
			}
#endif
				msm_hs_request_clock_on_internal(&msm_uport->uport);
			}
			msm_hs_submit_tx_locked(uport);
		}
	}
				    ...
				    unsigned int mctrl)
{
...
#ifdef	LOG_OUT
	{
		...
		if (msm_uport->dumpsize) {
			...
			printk("%s: set RTS to %s\n", tty->name, set_rts ? "OFF" : "ON");

			rxfs = msm_hs_read(uport, UARTDM_RXFS_ADDR);
			txfs = msm_hs_read(uport, UARTDM_TXFS_ADDR);

			if (txfs) {
				printk("%s: TXFS=0x%08x,Async=%d,Buffer=%d,State=%d\n", __func__, txfs,
					(txfs >> 10) & 0x0f, (txfs >> 7) & 0x07, (txfs & 0x7f) | (txfs >> 14));
			}
			if (rxfs) {
				printk("%s: RXFS=0x%08x,Async=%d,Buffer=%d,State=%d\n", __func__, rxfs,
...
#ifdef	CONFIG_PHS_MDM
int	msm_hs_ioctl(struct uart_port *uport, unsigned int cmd, unsigned long arg)
{
	int ret = -ENOIOCTLCMD;
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);

	if (cmd == SHPHS_CLK_ONOFF) {
		if (arg == 1) {
#ifdef	LOG_OUT
			if (msm_uport->dumpsize) {
				struct tty_struct *tty = msm_uport->uport.state->port.tty;
				printk("%s: %s()->msm_hs_request_clock_on()\n", tty->name, __func__);
			}
#endif
			msm_hs_request_clock_on(&msm_uport->uport);
		} else {
			...
			if (msm_uport->dumpsize) {
				struct tty_struct *tty = msm_uport->uport.state->port.tty;
				printk("%s: %s()->msm_hs_request_clock_off()\n", tty->name, __func__);
			}
#endif
			msm_hs_request_clock_off(&msm_uport->uport);
		}
		ret = 0;
	} else if (cmd == SHPHS_PURGE_SEND) {
		if (arg == 1) {
			msm_uport->purge_send = 1;
		} else {
			msm_uport->purge_send = 0;
		}
#ifdef	LOG_OUT
		if (msm_uport->dumpsize) {
			struct tty_struct *tty = msm_uport->uport.state->port.tty;
			printk("%s: %s()->purge_send=%d\n", tty->name, __func__, msm_uport->purge_send);
		}
#endif
		ret = 0;
	}
...
#ifdef	CONFIG_PHS_MDM
	if (pdata && (pdata->tx_buf_size > 0)) {
		msm_uport->tx_buf_size = pdata->tx_buf_size;
	} else {
		msm_uport->tx_buf_size = UARTDM_TX_BUF_SIZE;
	}
	printk(KERN_INFO "msm_serial_hs.%d: tx_buf_size=%d\n", pdev->id, msm_uport->tx_buf_size);
#endif	/* CONFIG_PHS_MDM */
