...
static struct msm_actuator_ctrl_t bu64291_actuator_t;

#define AF_DIRECT_MODE_ADDR 0xC4
#define IMX091_TOTAL_STEPS_NEAR_TO_FAR_MAX		40

int32_t bu64291_actuator_init_step_table(struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_set_info_t *set_info);

static struct msm_actuator bu64291_vcm_actuator_table = {
	.act_type = ACTUATOR_VCM,
	.func_tbl = {
//		.actuator_init_step_table = msm_actuator_init_step_table,
		.actuator_init_step_table  = bu64291_actuator_init_step_table,
...
static struct msm_actuator *actuators[] = {
	&bu64291_vcm_actuator_table,
};

int32_t bu64291_actuator_power_down(struct msm_actuator_ctrl_t *a_ctrl)
{
	int32_t rc = 0;
...
int32_t bu64291_actuator_init(struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_set_info_t *set_info) {
	...
	struct msm_actuator_set_info_t *set_info)
{
	...
	if(bu64291_actuator_t.af_adjustflg == 0x55){
...
int32_t bu64291_actuator_config(struct msm_actuator_ctrl_t *a_ctrl,
							void __user *argp)
{
	...
	case CFG_GET_ACTUATOR_INFO:
		cdata.cfg.get_info.step_shido = bu64291_actuator_t.step_shido;
		cdata.cfg.get_info.step_100_up = bu64291_actuator_t.step_100_up;
		cdata.cfg.get_info.step_500_up = bu64291_actuator_t.step_500_up;
		cdata.cfg.get_info.step_shido_up = bu64291_actuator_t.step_shido_up;
	...
	case CFG_SET_ACTUATOR_INFO:
		rc = bu64291_actuator_init(a_ctrl, &cdata.cfg.set_info);
		if (rc < 0)
	...
	case CFG_SHDIAG_ACTUATOR_SET_TABLE:
...
#if 1
		{
			...
			if(lens_pos >= bu64291_actuator_t.initial_code && 
			   lens_pos <= bu64291_actuator_t.terminal_code )
			{
				...
				msm_camera_i2c_write(&bu64291_actuator_t.i2c_client,
					codeval_msb,
					codeval_lsb,
					MSM_CAMERA_I2C_BYTE_DATA);

#if 1  /* init_lens_mod-> */
			    if(lens_pos - bu64291_actuator_t.step_position_table[0] <
			       bu64291_actuator_t.step_position_table[IMX091_TOTAL_STEPS_NEAR_TO_FAR_MAX-1] - lens_pos){
					bu64291_actuator_t.curr_step_pos = 0;
				}
				else{
					bu64291_actuator_t.curr_step_pos = IMX091_TOTAL_STEPS_NEAR_TO_FAR_MAX-1;
				}
		    	LINFO("%s curr_step_pos : %d\n", __func__ , bu64291_actuator_t.curr_step_pos);
#endif  /* init_lens_mod<- */
			}
...
#if 1
		{
			...
			if(p_sh_smem_common_type != NULL){
				...
				if(cdata.cfg.i2c_info.addr == 0x00) {
					bu64291_actuator_t.af_adjustflg = p_sh_smem_common_type->sh_camOtpData[0];
					if(bu64291_actuator_t.af_adjustflg == 0x55){
						bu64291_actuator_t.step_100_up =   ((p_sh_smem_common_type->sh_camOtpData[2] & 0x30) << 4) + p_sh_smem_common_type->sh_camOtpData[3];
						bu64291_actuator_t.step_500_up =   ((p_sh_smem_common_type->sh_camOtpData[2] & 0x0c) << 6) + p_sh_smem_common_type->sh_camOtpData[4];
						bu64291_actuator_t.step_shido =    ((p_sh_smem_common_type->sh_camOtpData[2] & 0x03) << 8) + p_sh_smem_common_type->sh_camOtpData[5];
						bu64291_actuator_t.step_shido_up = ((p_sh_smem_common_type->sh_camOtpData[2] & 0xc0) << 2) + p_sh_smem_common_type->sh_camOtpData[6];
						CDBG("sh_camOtpData step_100_up=%d, step_500_up=%d, step_shido=%d, step_shido_up=%d\n",
						         bu64291_actuator_t.step_100_up, bu64291_actuator_t.step_500_up, bu64291_actuator_t.step_shido_up, bu64291_actuator_t.step_shido_up);
	...
	case CFG_SHDIAG_ACTUATOR_SET_SMEM:
		{
			...
			if(cdata.cfg.i2c_info.addr == 0x00) {
				bu64291_actuator_t.af_adjustflg = wk_data[0];
				if(bu64291_actuator_t.af_adjustflg == 0x55){
					bu64291_actuator_t.step_100_up =   ((wk_data[2] & 0x30) << 4) + wk_data[3];
					bu64291_actuator_t.step_500_up =   ((wk_data[2] & 0x0c) << 6) + wk_data[4];
					bu64291_actuator_t.step_shido_up =    ((wk_data[2] & 0x03) << 8) + wk_data[5];
					bu64291_actuator_t.step_shido = ((wk_data[2] & 0xc0) << 2) + wk_data[6];
					CDBG("sh_camOtpData step_100_up=%d, step_500_up=%d, step_shido=%d, step_shido_up=%d\n",
					         bu64291_actuator_t.step_100_up, bu64291_actuator_t.step_500_up, bu64291_actuator_t.step_shido, bu64291_actuator_t.step_shido_up);
					CDBG("sh_camOtpData [0]=%02x, [2]=%02x, [3]=%02x, [4]=%02x [5]=%02x, [6]=%02x\n",
...
int32_t bu64291_actuator_i2c_probe(
	...
	const struct i2c_device_id *id)
{
	...
	if(p_sh_smem_common_type != NULL){
		...
		if(bu64291_actuator_t.af_adjustflg == 0x55){
			...
			bu64291_actuator_t.step_100_up =   ((p_sh_smem_common_type->sh_camOtpData[2] & 0x30) << 4) + p_sh_smem_common_type->sh_camOtpData[3];
			bu64291_actuator_t.step_500_up =   ((p_sh_smem_common_type->sh_camOtpData[2] & 0x0c) << 6) + p_sh_smem_common_type->sh_camOtpData[4];
			bu64291_actuator_t.step_shido_up =    ((p_sh_smem_common_type->sh_camOtpData[2] & 0x03) << 8) + p_sh_smem_common_type->sh_camOtpData[5];
			bu64291_actuator_t.step_shido = ((p_sh_smem_common_type->sh_camOtpData[2] & 0xc0) << 2) + p_sh_smem_common_type->sh_camOtpData[6];
...
int32_t bu64291_actuator_power_up(struct msm_actuator_ctrl_t *a_ctrl)
{
	int rc = 0;
	CDBG("%s called\n", __func__);
...
DEFINE_MUTEX(bu64291_actuator_mutex);

static const struct i2c_device_id bu64291_actuator_i2c_id[] = {
	{"bu64291_act", (kernel_ulong_t)&bu64291_actuator_t},
	{ }
};

static struct i2c_driver bu64291_actuator_i2c_driver = {
	.id_table = bu64291_actuator_i2c_id,
	.probe  = bu64291_actuator_i2c_probe,
...
static int __init bu64291_actuator_i2c_add_driver(
	void)
{
	CDBG("%s called\n", __func__);
	return i2c_add_driver(bu64291_actuator_t.i2c_driver);
}

long bu64291_actuator_subdev_ioctl(struct v4l2_subdev *sd,
			unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_ACTUATOR_CFG:
		return bu64291_actuator_config(a_ctrl, argp);
	default:
		return -ENOIOCTLCMD;
	}
}

int32_t bu64291_actuator_power(struct v4l2_subdev *sd, int on)
{
	int rc = 0;
	struct msm_actuator_ctrl_t *a_ctrl = get_actrl(sd);
	mutex_lock(a_ctrl->actuator_mutex);
	if (on)
		rc = bu64291_actuator_power_up(a_ctrl);
	else
		rc = bu64291_actuator_power_down(a_ctrl);
	mutex_unlock(a_ctrl->actuator_mutex);
	return rc;
}

static struct v4l2_subdev_core_ops bu64291_actuator_subdev_core_ops = {
	.ioctl = bu64291_actuator_subdev_ioctl,
	.s_power = bu64291_actuator_power,
};

static struct v4l2_subdev_ops bu64291_actuator_subdev_ops = {
	.core = &bu64291_actuator_subdev_core_ops,
};

static struct msm_actuator_ctrl_t bu64291_actuator_t = {
	.i2c_driver = &bu64291_actuator_i2c_driver,
	.act_v4l2_subdev_ops = &bu64291_actuator_subdev_ops,
	...
	.initial_code = 0,
	.terminal_code = 0,
	.actuator_mutex = &bu64291_actuator_mutex,

};

subsys_initcall(bu64291_actuator_i2c_add_driver);
MODULE_DESCRIPTION("BU64291 ACTUATOR");
