/* Copyright (c) 2012, The Linux Foundation. All rights reserved.
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 and
 * only version 2 as published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 */
#include "msm_sensor.h"
#include "msm.h"
#include "msm_ispif.h"
#include <sharp/sh_smem.h>
#define SENSOR_NAME "imx091"
#define PLATFORM_DRIVER_NAME "msm_camera_imx091"
#define imx091_obj imx091_##obj

#define REG_DIGITAL_GAIN_GREEN_R  0x020E
#define REG_DIGITAL_GAIN_RED  0x0210
#define REG_DIGITAL_GAIN_BLUE  0x0212
#define REG_DIGITAL_GAIN_GREEN_B  0x0214

#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
static struct camera_vreg_t msm_8960_imx091_vreg_1[] = {
	{"cam_vaf", REG_LDO, 3000000, 3000000, 300000},
};
struct regulator **imx091_reg_ptr;
#endif /* CONFIG_MACH_TOR */

//the issue of white blink when camera starts.
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
#define P_INIT_LINE 1356  //(Bv3.5)
#define P_INIT_GAIN ((uint16_t)(256 - 256/ 1.23 +0.5)) //reg = 256 - 256/gain
#else /* CONFIG_MACH_TOR */
#define P_INIT_LINE 818  //(Bv3.5)
#define P_INIT_GAIN ((uint16_t)(256 - 256/ 2.1 +0.5)) //reg = 256 - 256/gain
#endif /* CONFIG_MACH_TOR */
#define P_INIT_FLEN 0x0636  //1590
#define PIL_H ((0xFF00 & P_INIT_LINE) >> 8)
#define PIL_L (0x00FF & P_INIT_LINE)
#define PIG_H ((0xFF00 & P_INIT_GAIN) >> 8)
#define PIG_L (0x00FF & P_INIT_GAIN)
#define PIF_H ((0xFF00 & P_INIT_FLEN) >> 8)
#define PIF_L (0x00FF & P_INIT_FLEN)

#define IMX091_SENSOR_MCLK_6P75HZ  6750000
#define IMX091_SENSOR_A_MCLK_27P4HZ  27428571

#define IMX091_SENSOR_OTP_DATA_SIZE 7104
static unsigned char imx091_otp_data[IMX091_SENSOR_OTP_DATA_SIZE];

DEFINE_MUTEX(imx091_mut);
static struct msm_sensor_ctrl_t imx091_s_ctrl;
static uint8_t imx091_curr_mode = 10;
static int imx091_ae_lock=0;

static struct msm_camera_i2c_reg_conf imx091_start_settings[] = {
	{0x0100, 0x01},
};

static struct msm_camera_i2c_reg_conf imx091_stop_settings[] = {
	{0x0100, 0x00},
};

static struct msm_camera_i2c_reg_conf imx091_groupon_settings[] = {
	{0x0104, 0x01},
};

static struct msm_camera_i2c_reg_conf imx091_groupoff_settings[] = {
	{0x0104, 0x00},
};

static struct msm_camera_i2c_reg_conf imx091_prev_settings[] = {
	/* 30fps 1/1 * 1/2 */
	{0x0305,  0x01},
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	{0x0307,  0x53},
#else /* CONFIG_MACH_TOR */
	{0x0307,  0x54},
#endif /* CONFIG_MACH_TOR */
	{0x30A4,  0x02},
	{0x303C,  0x15},
	{0x0112,  0x0A},
	{0x0113,  0x0A},
	{0x0202,  PIL_H},
	{0x0203,  PIL_L},
	{0x0204,  PIG_H},
	{0x0205,  PIG_L},
	{0x0340,  PIF_H},
	{0x0341,  PIF_L},
	{0x0342,  0x12},
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	{0x0343,  0x0C},
#else /* CONFIG_MACH_TOR */
	{0x0343,  0x44},
#endif /* CONFIG_MACH_TOR */
	{0x0344,  0x00},
	{0x0345,  0x08},
	{0x0346,  0x00},
	{0x0347,  0x30},
	{0x0348,  0x10},
	{0x0349,  0x77},
	{0x034A,  0x0C},
	{0x034B,  0x5F},
	{0x034C,  0x10},
	{0x034D,  0x70},
	{0x034E,  0x06},
	{0x034F,  0x18},
	{0x0381,  0x01},
	{0x0383,  0x01},
	{0x0385,  0x01},
	{0x0387,  0x03},
	{0x3033,  0x00},
	{0x303D,  0x10},
	{0x303E,  0xD0},
	{0x3040,  0x08},
	{0x3041,  0x97},
	{0x3048,  0x01},
	{0x304C,  0x7F},
	{0x304D,  0x04},
	{0x3064,  0x12},
	{0x309B,  0x28},
	{0x309E,  0x00},
	{0x30D5,  0x00},
	{0x30D6,  0x85},
	{0x30D7,  0x2A},
	{0x30D8,  0x64},
	{0x30D9,  0x89},
	{0x30DA,  0x00},
	{0x30DB,  0x00},
	{0x30DC,  0x00},
	{0x30DD,  0x00},
	{0x30DE,  0x00},
	{0x3102,  0x10},
	{0x3103,  0x44},
	{0x3104,  0x40},
	{0x3105,  0x00},
	{0x3106,  0x0D},
	{0x3107,  0x01},
	{0x310A,  0x0A},
	{0x315C,  0x99},
	{0x315D,  0x98},
	{0x316E,  0x9A},
	{0x316F,  0x99},
	{0x3318,  0x64},
};

static struct msm_camera_i2c_reg_conf imx091_snap_settings[] = {
	{0x0305,  0x01},
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	{0x0307,  0x53},
#else /* CONFIG_MACH_TOR */
	{0x0307,  0x54},
#endif /* CONFIG_MACH_TOR */
	{0x30A4,  0x02},
	{0x303C,  0x15},
	{0x0112,  0x0A},
	{0x0113,  0x0A},
	{0x0340,  0x0C},
	{0x0341,  0xA0},
	{0x0342,  0x12},
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	{0x0343,  0x0C},
#else /* CONFIG_MACH_TOR */
	{0x0343,  0x44},
#endif /* CONFIG_MACH_TOR */
	{0x0344,  0x00},
	{0x0345,  0x08},
	{0x0346,  0x00},
	{0x0347,  0x30},
	{0x0348,  0x10},
	{0x0349,  0x77},
	{0x034A,  0x0C},
	{0x034B,  0x5F},
	{0x034C,  0x10},
	{0x034D,  0x70},
	{0x034E,  0x0C},
	{0x034F,  0x30},
	{0x0381,  0x01},
	{0x0383,  0x01},
	{0x0385,  0x01},
	{0x0387,  0x01},
	{0x3033,  0x00},
	{0x303D,  0x10},
	{0x303E,  0xD0},
	{0x3040,  0x08},
	{0x3041,  0x97},
	{0x3048,  0x00},
	{0x304C,  0x7F},
	{0x304D,  0x04},
	{0x3064,  0x12},
	{0x309B,  0x20},
	{0x309E,  0x00},
	{0x30D5,  0x00},
	{0x30D6,  0x85},
	{0x30D7,  0x2A},
	{0x30D8,  0x64},
	{0x30D9,  0x89},
	{0x30DA,  0x00},
	{0x30DB,  0x00},
	{0x30DC,  0x00},
	{0x30DD,  0x00},
	{0x30DE,  0x00},
	{0x3102,  0x10},
	{0x3103,  0x44},
	{0x3104,  0x40},
	{0x3105,  0x00},
	{0x3106,  0x0D},
	{0x3107,  0x01},
	{0x310A,  0x0A},
	{0x315C,  0x99},
	{0x315D,  0x98},
	{0x316E,  0x9A},
	{0x316F,  0x99},
	{0x3318,  0x64},
};

static struct msm_camera_i2c_reg_conf imx091_video_720p_60fps_settings[] = {
	{0x0305, 0x01},
	{0x0307, 0x53},
	{0x30A4, 0x02},
	{0x303C, 0x15},
	{0x0112, 0x0A},
	{0x0113, 0x0A},
	{0x0340, 0x03},
	{0x0341, 0x28},
	{0x0342, 0x12},
	{0x0343, 0x0C},
	{0x0344, 0x00},
	{0x0345, 0x08},
	{0x0346, 0x01},
	{0x0347, 0xB6},
	{0x0348, 0x10},
	{0x0349, 0x77},
	{0x034A, 0x0A},
	{0x034B, 0xD9},
	{0x034C, 0x10},
	{0x034D, 0x70},
	{0x034E, 0x03},
	{0x034F, 0x0C},
	{0x0381, 0x01},
	{0x0383, 0x01},
	{0x0385, 0x03},
	{0x0387, 0x03},
	{0x3033, 0x00},
	{0x303D, 0x10},
	{0x303E, 0xD0},
	{0x3040, 0x08},
	{0x3041, 0x97},
	{0x3048, 0x22},
	{0x304C, 0x7F},
	{0x304D, 0x04},
	{0x3064, 0x12},
	{0x309B, 0x60},
	{0x309E, 0x04},
	{0x30D5, 0x00},
	{0x30D6, 0x85},
	{0x30D7, 0x2A},
	{0x30D8, 0x64},
	{0x30D9, 0x89},
	{0x30DA, 0x00},
	{0x30DB, 0x00},
	{0x30DC, 0x00},
	{0x30DD, 0x00},
	{0x30DE, 0x00},
	{0x3102, 0x09},
	{0x3103, 0x23},
	{0x3104, 0x24},
	{0x3105, 0x00},
	{0x3106, 0x8B},
	{0x3107, 0x00},
	{0x310A, 0x0A},
	{0x315C, 0x4A},
	{0x315D, 0x49},
	{0x316E, 0x4B},
	{0x316F, 0x4A},
	{0x3318, 0x64},
};

static struct msm_camera_i2c_reg_conf imx091_video_vga_90fps_settings[] = {
	{0x0305, 0x01},
	{0x0307, 0x3D},
	{0x30A4, 0x02},
	{0x303C, 0x15},
	{0x0112, 0x0A},
	{0x0113, 0x0A},
	{0x0340, 0x03},
	{0x0341, 0x18},
	{0x0342, 0x09},
	{0x0343, 0x06},
	{0x0344, 0x00},
	{0x0345, 0x08},
	{0x0346, 0x00},
	{0x0347, 0x30},
	{0x0348, 0x10},
	{0x0349, 0x77},
	{0x034A, 0x0C},
	{0x034B, 0x5F},
	{0x034C, 0x08},
	{0x034D, 0x38},
	{0x034E, 0x02},
	{0x034F, 0x08},
	{0x0381, 0x01},
	{0x0383, 0x01},
	{0x0385, 0x05},
	{0x0387, 0x07},
	{0x3033, 0x84},
	{0x303D, 0x10},
	{0x303E, 0xD0},
	{0x3040, 0x08},
	{0x3041, 0x93},
	{0x3048, 0x01},
	{0x304C, 0x3F},
	{0x304D, 0x02},
	{0x3064, 0x12},
	{0x309B, 0x48},
	{0x309E, 0x04},
	{0x30D5, 0x04},
	{0x30D6, 0x85},
	{0x30D7, 0x2A},
	{0x30D8, 0x64},
	{0x30D9, 0x89},
	{0x30DA, 0x00},
	{0x30DB, 0x00},
	{0x30DC, 0x00},
	{0x30DD, 0x00},
	{0x30DE, 0x00},
	{0x3102, 0x09},
	{0x3103, 0x23},
	{0x3104, 0x24},
	{0x3105, 0x00},
	{0x3106, 0x8B},
	{0x3107, 0x00},
	{0x310A, 0x0A},
	{0x315C, 0x4A},
	{0x315D, 0x49},
	{0x316E, 0x4B},
	{0x316F, 0x4A},
	{0x3318, 0x66},
};

static struct msm_camera_i2c_reg_conf imx091_video_vga_120fps_settings[] = {
	{0x0305, 0x01},
	{0x0307, 0x3D},
	{0x30A4, 0x02},
	{0x303C, 0x15},
	{0x0112, 0x0A},
	{0x0113, 0x0A},
	{0x0340, 0x02},
	{0x0341, 0x52},
	{0x0342, 0x09},
	{0x0343, 0x06},
	{0x0344, 0x00},
	{0x0345, 0x08},
	{0x0346, 0x00},
	{0x0347, 0x30},
	{0x0348, 0x10},
	{0x0349, 0x77},
	{0x034A, 0x0C},
	{0x034B, 0x5F},
	{0x034C, 0x08},
	{0x034D, 0x38},
	{0x034E, 0x02},
	{0x034F, 0x08},
	{0x0381, 0x01},
	{0x0383, 0x01},
	{0x0385, 0x05},
	{0x0387, 0x07},
	{0x3033, 0x84},
	{0x303D, 0x10},
	{0x303E, 0xD0},
	{0x3040, 0x08},
	{0x3041, 0x93},
	{0x3048, 0x01},
	{0x304C, 0x3F},
	{0x304D, 0x02},
	{0x3064, 0x12},
	{0x309B, 0x48},
	{0x309E, 0x04},
	{0x30D5, 0x04},
	{0x30D6, 0x85},
	{0x30D7, 0x2A},
	{0x30D8, 0x64},
	{0x30D9, 0x89},
	{0x30DA, 0x00},
	{0x30DB, 0x00},
	{0x30DC, 0x00},
	{0x30DD, 0x00},
	{0x30DE, 0x00},
	{0x3102, 0x09},
	{0x3103, 0x23},
	{0x3104, 0x24},
	{0x3105, 0x00},
	{0x3106, 0x8B},
	{0x3107, 0x00},
	{0x310A, 0x0A},
	{0x315C, 0x4A},
	{0x315D, 0x49},
	{0x316E, 0x4B},
	{0x316F, 0x4A},
	{0x3318, 0x66},
};

static struct msm_camera_i2c_reg_conf imx091_recommend_settings[] = {
	{0x0101, 0x03},
	{0x3087, 0x53},
	{0x309D, 0x94},
	{0x30A1, 0x08},
	{0x30C7, 0x00},
	{0x3115, 0x0E},
	{0x3118, 0x42},
	{0x311D, 0x34},
	{0x3121, 0x0D},
	{0x3212, 0xF2},
	{0x3213, 0x0F},
	{0x3215, 0x0F},
	{0x3217, 0x0B},
	{0x3219, 0x0B},
	{0x321B, 0x0D},
	{0x321D, 0x0D},
	/* black level setting */
	{0x3032, 0x40},
};

static struct v4l2_subdev_info imx091_subdev_info[] = {
	{
	.code   = V4L2_MBUS_FMT_SBGGR10_1X10,
	.colorspace = V4L2_COLORSPACE_JPEG,
	.fmt    = 1,
	.order    = 0,
	},
	/* more can be supported, to be added later */
};

static struct msm_camera_i2c_conf_array imx091_init_conf[] = {
	{&imx091_recommend_settings[0],
	ARRAY_SIZE(imx091_recommend_settings), 0, MSM_CAMERA_I2C_BYTE_DATA}
};

static struct msm_camera_i2c_conf_array imx091_confs[] = {
	{&imx091_snap_settings[0],
	ARRAY_SIZE(imx091_snap_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&imx091_prev_settings[0],
	ARRAY_SIZE(imx091_prev_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&imx091_video_720p_60fps_settings[0],
	ARRAY_SIZE(imx091_video_720p_60fps_settings),
	0, MSM_CAMERA_I2C_BYTE_DATA},
	{&imx091_video_vga_90fps_settings[0],
	ARRAY_SIZE(imx091_video_vga_90fps_settings),
	0, MSM_CAMERA_I2C_BYTE_DATA},
	{&imx091_video_vga_120fps_settings[0],
	ARRAY_SIZE(imx091_video_vga_120fps_settings),
	0, MSM_CAMERA_I2C_BYTE_DATA},
};

static struct msm_sensor_output_info_t imx091_dimensions[] = {
	{
	/* full size */
		.x_output = 0x1070, /* 4208 */
		.y_output = 0x0C30, /* 3120 */
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
		.line_length_pclk = 0x120C, /* 4620 */
		.frame_length_lines = 0x0CA0, /* 3232 */
		.vt_pixel_clk = 224100000,
		.op_pixel_clk = 224100000,
#else /* CONFIG_MACH_TOR */
		.line_length_pclk = 0x1244, /* 4676 */
		.frame_length_lines = 0x0CA0, /* 3232 */
		.vt_pixel_clk = 226800000,
		.op_pixel_clk = 226800000,
#endif /* CONFIG_MACH_TOR */
		.binning_factor = 1,
	},
	{
	/* 30 fps 1/1 * 1/2 */
		.x_output = 0x1070, /* 4208 */
		.y_output = 0x0618, /* 1560 */
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
		.line_length_pclk = 0x120C, /* 4620 */
		.frame_length_lines = P_INIT_FLEN, /* 1590 30.5fps */
		.vt_pixel_clk = 224100000,
		.op_pixel_clk = 224100000,
#else /* CONFIG_MACH_TOR */
		.line_length_pclk = 0x1244, /* 4676 */
		.frame_length_lines = P_INIT_FLEN, /* 1590 30.5fps */
		.vt_pixel_clk = 226800000,
		.op_pixel_clk = 226800000,
#endif /* CONFIG_MACH_TOR */
		.binning_factor = 1,
	},
	{
	/* 720p 60fps 1/1 * 1/2 */
		.x_output = 0x1070, /* 4208 */
		.y_output = 0x030C, /* 780 */
		.line_length_pclk = 0x120C, /* 4620 */
		.frame_length_lines = 0x0328, /* 808*/
		.vt_pixel_clk = 224100000,
		.op_pixel_clk = 224100000,
		.binning_factor = 1,
	},
	{
	/* VGA 90fps 1/1 * 1/2 */
		.x_output = 0x0838, /* 2104 */
		.y_output = 0x0208, /* 520 */
		.line_length_pclk = 0x0906, /* 2310 */
		.frame_length_lines = 0x0318, /* 792*/
		.vt_pixel_clk = 164700000,
		.op_pixel_clk = 164700000,
		.binning_factor = 1,
	},
	{
	/* VGA 120fps 1/1 * 1/2 */
		.x_output = 0x0838, /* 2104 */
		.y_output = 0x0208, /* 520 */
		.line_length_pclk = 0x0906, /* 2310 */
		.frame_length_lines = 0x0252, /* 594*/
		.vt_pixel_clk = 164700000,
		.op_pixel_clk = 164700000,
		.binning_factor = 1,
	},
};

static struct msm_camera_csid_vc_cfg imx091_cid_cfg[] = {
	{0, CSI_RAW10, CSI_DECODE_10BIT},
	{1, CSI_EMBED_DATA, CSI_DECODE_8BIT},
	{2, CSI_RESERVED_DATA_0, CSI_DECODE_8BIT},
};

static struct msm_camera_csi2_params imx091_csi_params = {
	.csid_params = {
		.lane_cnt = 4,
		.lut_params = {
			.num_cid = ARRAY_SIZE(imx091_cid_cfg),
			.vc_cfg = imx091_cid_cfg,
		},
	},
	.csiphy_params = {
		.lane_cnt = 4,
		.settle_cnt = 0x12,
	},
};

static struct msm_camera_csi2_params *imx091_csi_params_array[] = {
	&imx091_csi_params,
	&imx091_csi_params,
	&imx091_csi_params,
	&imx091_csi_params,
	&imx091_csi_params,
	&imx091_csi_params,
	&imx091_csi_params,
	&imx091_csi_params,
};

static struct msm_sensor_output_reg_addr_t imx091_reg_addr = {
	.x_output = 0x034C,
	.y_output = 0x034E,
	.line_length_pclk = 0x0342,
	.frame_length_lines = 0x0340,
};

static struct msm_sensor_id_info_t imx091_id_info = {
	.sensor_id_reg_addr = 0x0000,
	.sensor_id = 0x0091,
};

static struct msm_sensor_exp_gain_info_t imx091_exp_gain_info = {
	.coarse_int_time_addr = 0x0202,
	.global_gain_addr = 0x0204,
	.vert_offset = 5,
};

typedef struct {
	uint32_t fl_lines;
	uint32_t line;
	uint16_t gain;
	uint16_t digital_gain;
} imx091_exp_gain_t;

imx091_exp_gain_t imx091_exp_gain[MSM_SENSOR_INVALID_RES] ={
	{ 0x0CA0, 0x0012, 0x000A, 0x0100},
	{ P_INIT_FLEN, P_INIT_LINE, P_INIT_GAIN, 0x0100},
};

static int imx091_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	struct sensor_cfg_data cdata;
	long   rc = 0;
	if (copy_from_user(&cdata,
		(void *)argp,
		sizeof(struct sensor_cfg_data)))
		return -EFAULT;
	mutex_lock(&imx091_mut);
	CDBG("imx091_sensor_config: cfgtype = %d\n",
	cdata.cfgtype);
	switch (cdata.cfgtype) {
	case CFG_SHDIAG_GET_I2C_DATA:
		{
			void *data;
			data = kmalloc(cdata.cfg.i2c_info.length, GFP_KERNEL);
			if(data == NULL){
				rc = -EFAULT;
				break;
			}
			CDBG("%s:%d i2c_read addr=0x%0x\n",__func__,__LINE__,cdata.cfg.i2c_info.addr);
			rc = msm_camera_i2c_read_seq(s_ctrl->sensor_i2c_client, cdata.cfg.i2c_info.addr , data, cdata.cfg.i2c_info.length);
			CDBG("%s:%d i2c_read data=0x%0x\n",__func__,__LINE__,*(unsigned char *)data);
			if (copy_to_user((void *)cdata.cfg.i2c_info.data,
				data,
				cdata.cfg.i2c_info.length)){
				kfree(data);
				CDBG("%s copy_to_user error\n",__func__);
				rc = -EFAULT;
				break;
			}
			kfree(data);
			if (copy_to_user((void *)argp,
				&cdata,
				sizeof(struct sensor_cfg_data))){
				rc = -EFAULT;
				break;
			}
		}
			break;
	case CFG_SHDIAG_SET_I2C_DATA:
		{
			void *data;
			data = kmalloc(cdata.cfg.i2c_info.length, GFP_KERNEL);
			if(data == NULL){
				rc = -EFAULT;
				break;
			}
			if (copy_from_user(data,
				(void *)cdata.cfg.i2c_info.data,
				cdata.cfg.i2c_info.length)){
				kfree(data);
				CDBG("%s copy_to_user error\n",__func__);
				rc = -EFAULT;
				break;
			}
			rc = msm_camera_i2c_write_seq(s_ctrl->sensor_i2c_client, cdata.cfg.i2c_info.addr, data, cdata.cfg.i2c_info.length);
			kfree(data);
			if (copy_to_user((void *)argp,
				&cdata,
				sizeof(struct sensor_cfg_data))){
				rc = -EFAULT;
				break;
			}
		}
			break;
	case CFG_SHDIAG_SET_SMEM:
		{
			void *data;
			data = kmalloc(cdata.cfg.i2c_info.length, GFP_KERNEL);
			if(data == NULL){
				rc = -EFAULT;
				break;
			}
			if (copy_from_user(data,
				(void *)cdata.cfg.i2c_info.data,
				cdata.cfg.i2c_info.length)){
				kfree(data);
				CDBG("%s copy_to_user error\n",__func__);
				rc = -EFAULT;
				break;
			}
			CDBG("cdata.cfg.i2c_info.addr = 0x%x\n", cdata.cfg.i2c_info.addr);
			memcpy(&imx091_otp_data[cdata.cfg.i2c_info.addr], (unsigned char *)data, cdata.cfg.i2c_info.length);

		    kfree(data);
			if (copy_to_user((void *)argp,
				&cdata,
				sizeof(struct sensor_cfg_data))) {
				rc = -EFAULT;
				break;
			}
		}
		break;
	case CFG_SHDIAG_GET_OTP_DATA:
		{
			CDBG("CFG_SHDIAG_GET_OTP_DATA IN\n");
			#if 0
			for(i = 0; i < IMX091_SENSOR_OTP_DATA_SIZE;i++)
				CDBG("imx091_otp_data[0][%d] = %02x\n ", i, p_sh_smem_common_type->sh_camOtpData[i]);
			#endif
			if(imx091_otp_data[0] != 0x55){
				CDBG("ISO/LB adjust value is not enable\n");
			}else{
				if(cdata.cfg.otp_info.length > IMX091_SENSOR_OTP_DATA_SIZE){
					pr_err("error cdata.cfg.otp_info.length\n");
					rc = -EFAULT;
					break;
				}
				if (copy_to_user((void *)cdata.cfg.otp_info.data,
					&imx091_otp_data[0],
					cdata.cfg.otp_info.length)){
					pr_err("%s copy_to_user error\n",__func__);
					rc = -EFAULT;
					break;
				}
			}
			if (copy_to_user((void *)argp,
				&cdata,
				sizeof(struct sensor_cfg_data))){
				rc = -EFAULT;
				break;
			}
		}
			break;
	case CFG_SH_SET_EXP_GAIN:
		{
			uint32_t fl_lines;
			uint8_t offset;
			long fps = 0;
			uint16_t ll_pclk;
			uint16_t cur_fl_lines;
			uint32_t delay = 0;

			fl_lines = s_ctrl->curr_frame_length_lines;
			fl_lines = (fl_lines * s_ctrl->fps_divider) / Q10;
			offset = s_ctrl->sensor_exp_gain_info->vert_offset;
			if (cdata.cfg.exp_gain.line > (fl_lines - offset))
				fl_lines = cdata.cfg.exp_gain.line + offset;

			if(imx091_ae_lock == 1){
				CDBG("%s imx091_ae_lock=%d", __func__, imx091_ae_lock);
				break;
			}

			if( imx091_curr_mode == 0 ) {
				rc = msm_camera_i2c_read(
						s_ctrl->sensor_i2c_client,
						s_ctrl->sensor_output_reg_addr->line_length_pclk, &ll_pclk,
						MSM_CAMERA_I2C_WORD_DATA);
				rc = msm_camera_i2c_read(
						s_ctrl->sensor_i2c_client,
						s_ctrl->sensor_output_reg_addr->frame_length_lines, &cur_fl_lines,
						MSM_CAMERA_I2C_WORD_DATA);
				fps = s_ctrl->msm_sensor_reg->
					output_settings[s_ctrl->curr_res].vt_pixel_clk /
					cur_fl_lines / ll_pclk;
				CDBG("%s ll_pclk = %d, frame fl_lines = %d\n", __func__, ll_pclk, cur_fl_lines);

				if(fps != 0)
					delay = 1000 / fps;
				CDBG("%s fps = %ld, delay = %d\n", __func__, fps, delay);
			}

			s_ctrl->func_tbl->sensor_group_hold_on(s_ctrl);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				s_ctrl->sensor_output_reg_addr->frame_length_lines, fl_lines,
				MSM_CAMERA_I2C_WORD_DATA);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				s_ctrl->sensor_exp_gain_info->coarse_int_time_addr, cdata.cfg.exp_gain.line,
				MSM_CAMERA_I2C_WORD_DATA);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				s_ctrl->sensor_exp_gain_info->global_gain_addr, cdata.cfg.exp_gain.gain,
				MSM_CAMERA_I2C_WORD_DATA);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				0x020E, cdata.cfg.exp_gain.digital_gain,
				MSM_CAMERA_I2C_WORD_DATA);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				0x0210, cdata.cfg.exp_gain.digital_gain,
				MSM_CAMERA_I2C_WORD_DATA);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				0x0212, cdata.cfg.exp_gain.digital_gain,
				MSM_CAMERA_I2C_WORD_DATA);
			msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				0x0214, cdata.cfg.exp_gain.digital_gain,
				MSM_CAMERA_I2C_WORD_DATA);
			s_ctrl->func_tbl->sensor_group_hold_off(s_ctrl);
			
			imx091_exp_gain[s_ctrl->curr_res].fl_lines = fl_lines;
			imx091_exp_gain[s_ctrl->curr_res].line = cdata.cfg.exp_gain.line;
			imx091_exp_gain[s_ctrl->curr_res].gain = cdata.cfg.exp_gain.gain;
			imx091_exp_gain[s_ctrl->curr_res].digital_gain = cdata.cfg.exp_gain.digital_gain;

			CDBG("%s fl_lines=%d line=%d gain=%d digital_gain=%d", __func__, fl_lines, cdata.cfg.exp_gain.line, cdata.cfg.exp_gain.gain, cdata.cfg.exp_gain.digital_gain);

			if( imx091_curr_mode == 0 ) {
				msleep(delay);
			}
		}
		break;
	case CFG_SHDIAG_AE_LOCK:
			imx091_ae_lock = cdata.cfg.ae_mode;
		break;
	default:
		mutex_unlock(&imx091_mut);
		return msm_sensor_config(s_ctrl, argp);
		break;
	}
	
	mutex_unlock(&imx091_mut);
	return rc;
}

int32_t imx091_sensor_write_exp_gain1(struct msm_sensor_ctrl_t *s_ctrl,
		uint16_t gain, uint32_t line)
{
	uint32_t fl_lines;
	uint8_t offset;
	fl_lines = s_ctrl->curr_frame_length_lines;
	fl_lines = (fl_lines * s_ctrl->fps_divider) / Q10;
	offset = s_ctrl->sensor_exp_gain_info->vert_offset;
	if (line > (fl_lines - offset))
		fl_lines = line + offset;

	s_ctrl->func_tbl->sensor_group_hold_on(s_ctrl);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_output_reg_addr->frame_length_lines, fl_lines,
		MSM_CAMERA_I2C_WORD_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_exp_gain_info->coarse_int_time_addr, line,
		MSM_CAMERA_I2C_WORD_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_exp_gain_info->global_gain_addr, gain,
		MSM_CAMERA_I2C_WORD_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		0x020E, imx091_exp_gain[s_ctrl->curr_res].digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		0x0210, imx091_exp_gain[s_ctrl->curr_res].digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		0x0212, imx091_exp_gain[s_ctrl->curr_res].digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		0x0214, imx091_exp_gain[s_ctrl->curr_res].digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);
	s_ctrl->func_tbl->sensor_group_hold_off(s_ctrl);
	return 0;
}

int32_t imx091_sensor_setting(struct msm_sensor_ctrl_t *s_ctrl,
			int update_type, int res)
{
	int32_t rc = 0;
	long fps = 0;
	uint16_t ll_pclk;
	uint16_t fl_lines;
	uint32_t delay = 0;

	if (update_type == MSM_SENSOR_REG_INIT) {
		s_ctrl->curr_csi_params = NULL;
		msm_sensor_enable_debugfs(s_ctrl);
		msm_sensor_write_init_settings(s_ctrl);
		imx091_exp_gain[0].fl_lines = 0x0CA0;
		imx091_exp_gain[0].line = 0x0012;
		imx091_exp_gain[0].gain = 0x000A;
		imx091_exp_gain[0].digital_gain = 0x0100;
		imx091_exp_gain[1].fl_lines = P_INIT_FLEN;
		imx091_exp_gain[1].line = P_INIT_LINE;
		imx091_exp_gain[1].gain = P_INIT_GAIN;
		imx091_exp_gain[1].digital_gain = 0x0100;

	} else if (update_type == MSM_SENSOR_UPDATE_PERIODIC) {
		if (s_ctrl->curr_res != MSM_SENSOR_INVALID_RES) {
			s_ctrl->func_tbl->sensor_stop_stream(s_ctrl);
			rc = msm_camera_i2c_read(
					s_ctrl->sensor_i2c_client,
					s_ctrl->sensor_output_reg_addr->line_length_pclk, &ll_pclk,
					MSM_CAMERA_I2C_WORD_DATA);
			rc = msm_camera_i2c_read(
					s_ctrl->sensor_i2c_client,
					s_ctrl->sensor_output_reg_addr->frame_length_lines, &fl_lines,
					MSM_CAMERA_I2C_WORD_DATA);
			CDBG("%s ll_pclk = %d, frame fl_lines = %d\n", __func__, ll_pclk, fl_lines);
			fps = s_ctrl->msm_sensor_reg->
				output_settings[s_ctrl->curr_res].vt_pixel_clk /
				fl_lines / ll_pclk;
			if(fps != 0)
				delay = 1000 / fps;
			CDBG("%s fps = %ld, frame time = %d\n", __func__, fps, delay);
//			delay += 40;
			CDBG("%s delay = %d\n", __func__, delay);
			msleep(delay);
		}

		msm_sensor_write_res_settings(s_ctrl, res);
		if (s_ctrl->curr_csi_params != s_ctrl->csi_params[res]) {
			s_ctrl->curr_csi_params = s_ctrl->csi_params[res];
			s_ctrl->curr_csi_params->csid_params.lane_assign =
				s_ctrl->sensordata->sensor_platform_info->
				csi_lane_params->csi_lane_assign;
			s_ctrl->curr_csi_params->csiphy_params.lane_mask =
				s_ctrl->sensordata->sensor_platform_info->
				csi_lane_params->csi_lane_mask;
			v4l2_subdev_notify(&s_ctrl->sensor_v4l2_subdev,
				NOTIFY_CSID_CFG,
				&s_ctrl->curr_csi_params->csid_params);
			mb();
			v4l2_subdev_notify(&s_ctrl->sensor_v4l2_subdev,
				NOTIFY_CSIPHY_CFG,
				&s_ctrl->curr_csi_params->csiphy_params);
			mb();
			msleep(20);
		}

		v4l2_subdev_notify(&s_ctrl->sensor_v4l2_subdev,
			NOTIFY_PCLK_CHANGE, &s_ctrl->msm_sensor_reg->
			output_settings[res].op_pixel_clk);
		s_ctrl->func_tbl->sensor_start_stream(s_ctrl);
		msleep(30);
	}
	return rc;
}

int32_t imx091_sensor_adjust_frame_lines(struct msm_sensor_ctrl_t *s_ctrl,
	uint16_t res)
{
	if(imx091_ae_lock == 1){
		CDBG("%s imx091_ae_lock=%d", __func__, imx091_ae_lock);
		return 0;
	}

	if((res == MSM_SENSOR_RES_QTR) || (res == MSM_SENSOR_RES_FULL)){
		s_ctrl->func_tbl->sensor_group_hold_on(s_ctrl);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_output_reg_addr->frame_length_lines, imx091_exp_gain[res].fl_lines,
			MSM_CAMERA_I2C_WORD_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_exp_gain_info->coarse_int_time_addr, imx091_exp_gain[res].line,
			MSM_CAMERA_I2C_WORD_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_exp_gain_info->global_gain_addr, imx091_exp_gain[res].gain,
			MSM_CAMERA_I2C_WORD_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			0x020E, imx091_exp_gain[res].digital_gain,
			MSM_CAMERA_I2C_WORD_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			0x0210, imx091_exp_gain[res].digital_gain,
			MSM_CAMERA_I2C_WORD_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			0x0212, imx091_exp_gain[res].digital_gain,
			MSM_CAMERA_I2C_WORD_DATA);
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			0x0214, imx091_exp_gain[res].digital_gain,
			MSM_CAMERA_I2C_WORD_DATA);
		s_ctrl->func_tbl->sensor_group_hold_off(s_ctrl);
		
		CDBG("%s fl_lines=%d line=%d gain=%d digital_gain=%d", __func__, imx091_exp_gain[res].fl_lines, imx091_exp_gain[res].line, imx091_exp_gain[res].gain, imx091_exp_gain[res].digital_gain);
	}
	return 0;
}

int32_t imx091_sensor_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	sharp_smem_common_type *p_sh_smem_common_type = NULL;

	p_sh_smem_common_type = sh_smem_get_common_address();
	if(p_sh_smem_common_type != NULL){
		memcpy(&imx091_otp_data[0], &p_sh_smem_common_type->sh_camOtpData[48], IMX091_SENSOR_OTP_DATA_SIZE);
	}

	return msm_sensor_i2c_probe(client, id);
}

extern int shcamled_pmic_set_torch_led_1_current(unsigned mA);

int imx091_pmic_set_flash_led_current(enum pmic8058_leds id, unsigned mA)
{
	int ret = 0;
	ret = shcamled_pmic_set_torch_led_1_current(mA);
	return ret;
}
EXPORT_SYMBOL(imx091_pmic_set_flash_led_current);

static const struct i2c_device_id imx091_i2c_id[] = {
	{SENSOR_NAME, (kernel_ulong_t)&imx091_s_ctrl},
	{ }
};

static struct i2c_driver imx091_i2c_driver = {
	.id_table = imx091_i2c_id,
	.probe  = imx091_sensor_i2c_probe,
	.driver = {
		.name = SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client imx091_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};

int32_t imx091_sensor_write_exp_gain(struct msm_sensor_ctrl_t *s_ctrl,
		uint32_t gain, uint32_t line)
{
	uint32_t fl_lines;
	uint8_t offset;
	static uint16_t max_analog_gain  = 0x00E0;
	uint16_t digital_gain = 0x0100;

	fl_lines = s_ctrl->curr_frame_length_lines;
	fl_lines = (fl_lines * s_ctrl->fps_divider) / Q10;
	offset = s_ctrl->sensor_exp_gain_info->vert_offset;
	if (line > (fl_lines - offset))
		fl_lines = line + offset;

	digital_gain = ((gain & 0xFFF00) >> 8);
	gain = (gain & 0x00FF);

	if (gain > max_analog_gain)
		gain = max_analog_gain;

	if (digital_gain < 0x0100)
		digital_gain = 0x0100;

	CDBG("digital_gain =0x%X Analog gain = %d\n", digital_gain, gain);
	s_ctrl->func_tbl->sensor_group_hold_on(s_ctrl);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_output_reg_addr->frame_length_lines, fl_lines,
		MSM_CAMERA_I2C_WORD_DATA);

	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_exp_gain_info->coarse_int_time_addr, line,
		MSM_CAMERA_I2C_WORD_DATA);

	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		s_ctrl->sensor_exp_gain_info->global_gain_addr, gain,
		MSM_CAMERA_I2C_WORD_DATA);

	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		REG_DIGITAL_GAIN_GREEN_R, digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);

	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		REG_DIGITAL_GAIN_RED, digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);

	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		REG_DIGITAL_GAIN_BLUE, digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);

	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
		REG_DIGITAL_GAIN_GREEN_B, digital_gain,
		MSM_CAMERA_I2C_WORD_DATA);

	s_ctrl->func_tbl->sensor_group_hold_off(s_ctrl);
	return 0;
}

static int32_t imx091_sensor_power(struct v4l2_subdev *sd, int on)
{
	int rc = 0;
	struct msm_sensor_ctrl_t *s_ctrl = get_sctrl(sd);
	mutex_lock(s_ctrl->msm_sensor_mutex);
	if (on) {
		rc = s_ctrl->func_tbl->sensor_power_up(s_ctrl);
		if (rc < 0) {
			pr_err("%s: %s power_up failed rc = %d\n", __func__,
				s_ctrl->sensordata->sensor_name, rc);
		}
	} else {
		rc = s_ctrl->func_tbl->sensor_power_down(s_ctrl);
	}
	mutex_unlock(s_ctrl->msm_sensor_mutex);
	return rc;
}

static int __init imx091_sensor_init_module(void)
{
	return i2c_add_driver(&imx091_i2c_driver);
}

static struct v4l2_subdev_core_ops imx091_subdev_core_ops = {
	.ioctl = msm_sensor_subdev_ioctl,
	.s_power = imx091_sensor_power,
};

static struct v4l2_subdev_video_ops imx091_subdev_video_ops = {
	.enum_mbus_fmt = msm_sensor_v4l2_enum_fmt,
};

static struct v4l2_subdev_ops imx091_subdev_ops = {
	.core = &imx091_subdev_core_ops,
	.video  = &imx091_subdev_video_ops,
};

static struct msm_cam_clk_info cam_clk_info[] = {
	{"cam_clk", IMX091_SENSOR_MCLK_6P75HZ},
#ifdef CONFIG_IMX091_ACT
	{"cam_a_clk", IMX091_SENSOR_A_MCLK_27P4HZ},
#endif
};

int32_t imx091_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	struct msm_camera_sensor_info *data = s_ctrl->sensordata;
	int i;

	CDBG("%s: %d\n", __func__, __LINE__);

	s_ctrl->reg_ptr = kzalloc(sizeof(struct regulator *)
			* data->sensor_platform_info->num_vreg, GFP_KERNEL);
	if (!s_ctrl->reg_ptr) {
		pr_err("%s: could not allocate mem for regulators\n",
			__func__);
		return -ENOMEM;
	}
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	imx091_reg_ptr = kzalloc(sizeof(struct regulator *)
			* ARRAY_SIZE(msm_8960_imx091_vreg_1), GFP_KERNEL);
	if (!imx091_reg_ptr) {
		pr_err("%s: could not allocate mem for imx091_reg_ptr\n",
			__func__);
		kfree(s_ctrl->reg_ptr);
		return -ENOMEM;
	}
#endif /* CONFIG_MACH_TOR */
	rc = msm_camera_request_gpio_table(data, 1);
	if (rc < 0) {
		pr_err("%s: request gpio failed\n", __func__);
		goto request_gpio_failed;
	}

	rc = msm_camera_config_gpio_table(data, 0);
	if (rc < 0) {
		pr_err("%s: config gpio dis failed\n", __func__);
		goto config_gpio_dis_failed;
	}

	rc = msm_camera_config_vreg(&s_ctrl->sensor_i2c_client->client->dev,
			s_ctrl->sensordata->sensor_platform_info->cam_vreg,
			s_ctrl->sensordata->sensor_platform_info->num_vreg,
			s_ctrl->reg_ptr, 1);
	if (rc < 0) {
		pr_err("%s: regulator on failed\n", __func__);
		goto config_vreg_failed;
	}

	rc = msm_camera_enable_vreg(&s_ctrl->sensor_i2c_client->client->dev,
			s_ctrl->sensordata->sensor_platform_info->cam_vreg,
			s_ctrl->sensordata->sensor_platform_info->num_vreg,
			s_ctrl->reg_ptr, 1);
	if (rc < 0) {
		pr_err("%s: enable regulator failed\n", __func__);
		goto enable_vreg_failed;
	}

#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	usleep_range(800,1800);

	rc = msm_camera_config_vreg(&s_ctrl->sensor_i2c_client->client->dev,
			msm_8960_imx091_vreg_1,
			ARRAY_SIZE(msm_8960_imx091_vreg_1),
			imx091_reg_ptr, 1);
	if (rc < 0) {
		pr_err("%s: regulator on failed\n", __func__);
		goto config_vreg_failed;
	}

	rc = msm_camera_enable_vreg(&s_ctrl->sensor_i2c_client->client->dev,
			msm_8960_imx091_vreg_1,
			ARRAY_SIZE(msm_8960_imx091_vreg_1),
			imx091_reg_ptr, 1);
	if (rc < 0) {
		pr_err("%s: enable regulator failed\n", __func__);
		goto enable_vreg_failed;
	}
#else /* CONFIG_MACH_TOR */
	usleep_range(1000,2000);
#endif /* CONFIG_MACH_TOR */

	for (i = 0; i < data->sensor_platform_info->gpio_conf->cam_gpio_set_tbl_size; i++) {
		gpio_set_value_cansleep(
			data->sensor_platform_info->gpio_conf->cam_gpio_set_tbl[i].gpio,
			data->sensor_platform_info->gpio_conf->cam_gpio_set_tbl[i].flags);
	}

#if defined(CONFIG_MACH_AS70) 
	usleep_range(1000,2000);
#endif /* CONFIG_MACH_AS70 */

	if (s_ctrl->clk_rate != 0)
		cam_clk_info->clk_rate = s_ctrl->clk_rate;

	rc = msm_cam_clk_enable(&s_ctrl->sensor_i2c_client->client->dev,
		cam_clk_info, &s_ctrl->cam_clk, ARRAY_SIZE(cam_clk_info), 1);
	if (rc < 0) {
		pr_err("%s: clk enable failed\n", __func__);
		goto enable_clk_failed;
	}
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	usleep_range(300,1300);
#else /* CONFIG_MACH_TOR */
	usleep_range(1000,2000);
#endif /* CONFIG_MACH_TOR */

	imx091_curr_mode = 10;
	imx091_ae_lock = 0;

	return rc;

enable_clk_failed:
		msm_camera_config_gpio_table(data, 0);
//config_gpio_failed:
	msm_camera_enable_vreg(&s_ctrl->sensor_i2c_client->client->dev,
			s_ctrl->sensordata->sensor_platform_info->cam_vreg,
			s_ctrl->sensordata->sensor_platform_info->num_vreg,
			s_ctrl->reg_ptr, 0);

enable_vreg_failed:
	msm_camera_config_vreg(&s_ctrl->sensor_i2c_client->client->dev,
		s_ctrl->sensordata->sensor_platform_info->cam_vreg,
		s_ctrl->sensordata->sensor_platform_info->num_vreg,
		s_ctrl->reg_ptr, 0);
config_vreg_failed:
	msm_camera_config_gpio_table(data, 0);
config_gpio_dis_failed:
	msm_camera_request_gpio_table(data, 0);
request_gpio_failed:
	kfree(s_ctrl->reg_ptr);
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	kfree(imx091_reg_ptr);
	imx091_reg_ptr = NULL;
#endif /* CONFIG_MACH_TOR */
	CDBG("%s: %d\n", __func__, __LINE__);
	return rc;
}

int32_t imx091_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	long fps = 0;
	uint16_t ll_pclk;
	uint16_t fl_lines;
	uint32_t delay = 0;
	struct msm_camera_sensor_info *data = s_ctrl->sensordata;
	CDBG("%s\n", __func__);

	s_ctrl->func_tbl->sensor_stop_stream(s_ctrl);

	rc = msm_camera_i2c_read(
			s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_output_reg_addr->line_length_pclk, &ll_pclk,
			MSM_CAMERA_I2C_WORD_DATA);
	rc = msm_camera_i2c_read(
			s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_output_reg_addr->frame_length_lines, &fl_lines,
			MSM_CAMERA_I2C_WORD_DATA);
	CDBG("%s ll_pclk = %d, frame fl_lines = %d\n", __func__, ll_pclk, fl_lines);

	if((ll_pclk != 0) && (fl_lines != 0)) {
		s_ctrl->func_tbl->sensor_stop_stream(s_ctrl);

		fps = s_ctrl->msm_sensor_reg->
			output_settings[s_ctrl->curr_res].vt_pixel_clk /
			fl_lines / ll_pclk;
		if(fps != 0)
			delay = 1000 / fps;
		CDBG("%s fps = %ld, frame time = %d\n", __func__, fps, delay);
		msleep(delay);
	}

	msm_cam_clk_enable(&s_ctrl->sensor_i2c_client->client->dev,
		cam_clk_info, &s_ctrl->cam_clk, ARRAY_SIZE(cam_clk_info), 0);

	msm_camera_config_gpio_table(data, 0);
	msleep(1);

	msm_camera_enable_vreg(&s_ctrl->sensor_i2c_client->client->dev,
		s_ctrl->sensordata->sensor_platform_info->cam_vreg,
		s_ctrl->sensordata->sensor_platform_info->num_vreg,
		s_ctrl->reg_ptr, 0);
	msm_camera_config_vreg(&s_ctrl->sensor_i2c_client->client->dev,
		s_ctrl->sensordata->sensor_platform_info->cam_vreg,
		s_ctrl->sensordata->sensor_platform_info->num_vreg,
		s_ctrl->reg_ptr, 0);
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	msm_camera_enable_vreg(&s_ctrl->sensor_i2c_client->client->dev,
		msm_8960_imx091_vreg_1,
		ARRAY_SIZE(msm_8960_imx091_vreg_1),
		imx091_reg_ptr, 0);
	msm_camera_config_vreg(&s_ctrl->sensor_i2c_client->client->dev,
		msm_8960_imx091_vreg_1,
		ARRAY_SIZE(msm_8960_imx091_vreg_1),
		imx091_reg_ptr, 0);
#endif /* CONFIG_MACH_TOR */
	msm_camera_request_gpio_table(data, 0);
	kfree(s_ctrl->reg_ptr);
#if defined(CONFIG_MACH_TOR) || defined(CONFIG_MACH_MNB)
	kfree(imx091_reg_ptr);
	imx091_reg_ptr = NULL;
#endif /* CONFIG_MACH_TOR */
	CDBG("%s\n", __func__);
	return 0;
}

static int32_t imx091_sensor_set_sensor_mode(struct msm_sensor_ctrl_t *s_ctrl,
	int mode, int res)
{
	int32_t rc = 0;

	CDBG("%s mode=%d \n", __func__, mode);
	if(((imx091_curr_mode != 9) && (mode == 9)) ||
	   ((imx091_curr_mode == 9) && (mode == 2))){
		CDBG("%s mode==SENSOR_MODE_ZSL or SENSOR_MODE_ZSL -> SENSOR_MODE_PREVIEW\n", __func__);
		imx091_curr_mode = mode;

		imx091_exp_gain[0].fl_lines = 0x0CA0;
		imx091_exp_gain[0].line = 0x0012;
		imx091_exp_gain[0].gain = 0x000A;
		imx091_exp_gain[0].digital_gain = 0x0100;
		imx091_exp_gain[1].fl_lines = P_INIT_FLEN;
		imx091_exp_gain[1].line = P_INIT_LINE;
		imx091_exp_gain[1].gain = P_INIT_GAIN;
		imx091_exp_gain[1].digital_gain = 0x0100;

		if (s_ctrl->curr_res != res) {
			s_ctrl->curr_frame_length_lines =
				s_ctrl->msm_sensor_reg->
				output_settings[res].frame_length_lines;

			s_ctrl->curr_line_length_pclk =
				s_ctrl->msm_sensor_reg->
				output_settings[res].line_length_pclk;

			if (s_ctrl->is_csic ||
				!s_ctrl->sensordata->csi_if)
				rc = s_ctrl->func_tbl->sensor_csi_setting(s_ctrl,
					MSM_SENSOR_UPDATE_PERIODIC, res);
			else
				rc = s_ctrl->func_tbl->sensor_setting(s_ctrl,
					MSM_SENSOR_UPDATE_PERIODIC, res);
			if (rc < 0)
				return rc;
			s_ctrl->curr_res = res;
		} else {
			long fps = 0;
			uint16_t ll_pclk;
			uint16_t fl_lines;
			uint32_t delay = 0;

			CDBG("%s res=%d s_ctrl->curr_res=%d\n", __func__, res, s_ctrl->curr_res);

			rc = msm_camera_i2c_read(
					s_ctrl->sensor_i2c_client,
					s_ctrl->sensor_output_reg_addr->line_length_pclk, &ll_pclk,
					MSM_CAMERA_I2C_WORD_DATA);
			rc = msm_camera_i2c_read(
					s_ctrl->sensor_i2c_client,
					s_ctrl->sensor_output_reg_addr->frame_length_lines, &fl_lines,
					MSM_CAMERA_I2C_WORD_DATA);
			CDBG("%s ll_pclk = %d, frame fl_lines = %d\n", __func__, ll_pclk, fl_lines);

			if (s_ctrl->func_tbl->sensor_adjust_frame_lines)
				rc = s_ctrl->func_tbl->sensor_adjust_frame_lines(s_ctrl, res);

			fps = s_ctrl->msm_sensor_reg->
				output_settings[s_ctrl->curr_res].vt_pixel_clk /
				fl_lines / ll_pclk;
			if(fps != 0)
				delay = 1000 / fps;
			CDBG("%s fps = %ld, frame time = %d\n", __func__, fps, delay);
			delay += 10;
			CDBG("%s delay = %d\n", __func__, delay);
			msleep(delay);

		}
	} else {
		CDBG("%s mode!=SENSOR_MODE_ZSL \n", __func__);
		imx091_curr_mode = mode;
		if (s_ctrl->curr_res != res) {
			s_ctrl->curr_frame_length_lines =
				s_ctrl->msm_sensor_reg->
				output_settings[res].frame_length_lines;

			s_ctrl->curr_line_length_pclk =
				s_ctrl->msm_sensor_reg->
				output_settings[res].line_length_pclk;

			if (s_ctrl->is_csic ||
				!s_ctrl->sensordata->csi_if)
				rc = s_ctrl->func_tbl->sensor_csi_setting(s_ctrl,
					MSM_SENSOR_UPDATE_PERIODIC, res);
			else
				rc = s_ctrl->func_tbl->sensor_setting(s_ctrl,
					MSM_SENSOR_UPDATE_PERIODIC, res);
			if (rc < 0)
				return rc;
			s_ctrl->curr_res = res;
		}
	}

	return rc;
}

static struct msm_sensor_fn_t imx091_func_tbl = {
	.sensor_start_stream = msm_sensor_start_stream,
	.sensor_stop_stream = msm_sensor_stop_stream,
	.sensor_group_hold_on = msm_sensor_group_hold_on,
	.sensor_group_hold_off = msm_sensor_group_hold_off,
	.sensor_set_fps = msm_sensor_set_fps,
	.sensor_write_exp_gain = imx091_sensor_write_exp_gain1,
	.sensor_write_snapshot_exp_gain = imx091_sensor_write_exp_gain1,
	.sensor_setting = imx091_sensor_setting,
	.sensor_set_sensor_mode = imx091_sensor_set_sensor_mode,
	.sensor_mode_init = msm_sensor_mode_init,
	.sensor_get_output_info = msm_sensor_get_output_info,
	.sensor_config = imx091_sensor_config,
	.sensor_power_up = imx091_power_up,
	.sensor_power_down = imx091_power_down,
	.sensor_adjust_frame_lines = imx091_sensor_adjust_frame_lines,
	.sensor_get_csi_params = msm_sensor_get_csi_params,
};

static struct msm_sensor_reg_t imx091_regs = {
	.default_data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.start_stream_conf = imx091_start_settings,
	.start_stream_conf_size = ARRAY_SIZE(imx091_start_settings),
	.stop_stream_conf = imx091_stop_settings,
	.stop_stream_conf_size = ARRAY_SIZE(imx091_stop_settings),
	.group_hold_on_conf = imx091_groupon_settings,
	.group_hold_on_conf_size = ARRAY_SIZE(imx091_groupon_settings),
	.group_hold_off_conf = imx091_groupoff_settings,
	.group_hold_off_conf_size = ARRAY_SIZE(imx091_groupoff_settings),
	.init_settings = &imx091_init_conf[0],
	.init_size = ARRAY_SIZE(imx091_init_conf),
	.mode_settings = &imx091_confs[0],
	.output_settings = &imx091_dimensions[0],
	.num_conf = ARRAY_SIZE(imx091_confs),
};

static struct msm_sensor_ctrl_t imx091_s_ctrl = {
	.msm_sensor_reg = &imx091_regs,
	.sensor_i2c_client = &imx091_sensor_i2c_client,
	.sensor_i2c_addr = 0x34,
	.sensor_output_reg_addr = &imx091_reg_addr,
	.sensor_id_info = &imx091_id_info,
	.sensor_exp_gain_info = &imx091_exp_gain_info,
	.cam_mode = MSM_SENSOR_MODE_INVALID,
	.csi_params = &imx091_csi_params_array[0],
	.msm_sensor_mutex = &imx091_mut,
	.sensor_i2c_driver = &imx091_i2c_driver,
	.sensor_v4l2_subdev_info = imx091_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(imx091_subdev_info),
	.sensor_v4l2_subdev_ops = &imx091_subdev_ops,
	.func_tbl = &imx091_func_tbl,
	.clk_rate = IMX091_SENSOR_MCLK_6P75HZ,
};

module_init(imx091_sensor_init_module);
MODULE_DESCRIPTION("SONY 12MP Bayer sensor driver");
MODULE_LICENSE("GPL v2");
