...
/* alpha2.1 patch -> */
	if (vfe32_ctrl->update_abcc) {
		value = msm_camera_io_r(vfe32_ctrl->vfebase + V32_DEMOSAICV3_0_OFF);
		...
		msm_camera_io_w(value, vfe32_ctrl->vfebase + V32_DEMOSAICV3_0_OFF);
...
#if 1 /* custom */
	if (pmctl->mctl_release == NULL) {
		pr_err("%s error released [mctl_release]", __func__);
		rc = -EINVAL;
		return rc;	
	}
#endif
...
/* alpha2.1 patch -> */
	...
	case VFE_CMD_DEMOSAICV3_ABCC_CFG: {
		...
		old_val = msm_camera_io_r(vfe32_ctrl->vfebase + V32_DEMOSAICV3_0_OFF);
		old_val &= ABCC_MASK;
		new_val = new_val | old_val;
		*cmdp_local = new_val;
