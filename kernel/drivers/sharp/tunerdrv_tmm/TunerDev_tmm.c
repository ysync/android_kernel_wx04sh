...
static int tuner_vreg_enable(void)
{
	...
	/* for ADTV (2.2V) */
	struct regulator *reg_22v;
	struct device *dev_22v = NULL;
	const char *id_22v = "8921_s8";
	int min_22v_uV = 2200000, max_22v_uV = 2200000;

	reg_22v= regulator_get(dev_22v, id_22v);
	if (IS_ERR(reg_22v)) {
		printk("Unable to get %s regulator\n", id_22v);
		return -1;
	}

    regulator_set_voltage(reg_22v, min_22v_uV, max_22v_uV);

	if (!regulator_is_enabled(reg_22v)) {
		regulator_enable(reg_22v);
	}

	regulator_put(reg_22v);
	/* --- */
