...
#define SUCC_PON_WAIT_ENABLE		(1)
#define SUCC_PON_WAIT_DISABLE		(0)
...
static int g_pon_wait_flg = SUCC_PON_WAIT_ENABLE;
...
static long succ_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SHSNFC_UCC_REQ_START_NFC_P2P:
	...
	if (cmd == SHSNFC_UCC_REQ_START_NFC_P2P) {
		if ((unsigned int)result & (unsigned int)SHSNFC_UCC_RFS_CHECK) {
			g_pon_wait_flg = (D_RFS_DEV_LOW == get_rfs_value()) ? SUCC_PON_WAIT_DISABLE : SUCC_PON_WAIT_ENABLE;
		} else {
			g_pon_wait_flg = SUCC_PON_WAIT_DISABLE;
		}
	} else {
		g_pon_wait_flg = SUCC_PON_WAIT_ENABLE;
	}
	MFC_DRV_DBG_LOG("g_pon_wait_flg = %d", g_pon_wait_flg);

	if (((unsigned int)result & (unsigned int)SHSNFC_UCC_RWS_CHECK) && (mfc_rws_sts == D_RWS_RW_DISABLE)) {
