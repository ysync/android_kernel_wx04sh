...
//#define SH_MHL_I2C_SEQUENCE_LOG

#ifdef SH_MHL_I2C_SEQUENCE_LOG
	#define SH_MHL_I2C_LOG(x, ...)	printk(x, __VA_ARGS__)
#else
	#define SH_MHL_I2C_LOG(x, ...)
#endif /* SH_MHL_I2C_SEQUENCE_LOG */
...
#ifdef CONFIG_SII8334_MHL_TX
	SH_MHL_I2C_LOG("I2C slv addr[0x%x] offset[0x%x] cnt[%d]  ---->\n", deviceId, regAddr, count);
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
	SH_MHL_I2C_LOG("I2C read result[0x%x] Sts[%d]  <----\n",*pBuffer, status);
#endif /* CONFIG_SII8334_MHL_TX */
    return( status );
}
...
#ifdef CONFIG_SII8334_MHL_TX
	SH_MHL_I2C_LOG("I2C write [0x%x] Sts[%d]  <----\n",*pBuffer, status);
#endif /* CONFIG_SII8334_MHL_TX */
    return( status );
}
