...
static int32_t MhlI2cProbe(struct i2c_client *client, const struct i2c_device_id *id)
{
	SII_DEBUG_PRINT(SII_OSAL_DEBUG_TRACE, "in probe\n");
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		SII_DEBUG_PRINT(SII_OSAL_DEBUG_TRACE, "client not i2c capable\n");
		}
	else
		SII_DEBUG_PRINT(SII_OSAL_DEBUG_TRACE, "client is i2c capable\n");
...
halReturn_t HalOpenI2cDevice(char const *DeviceName, char const *DriverName)
{
    ...
    //RG addition to try again in 10 seconds...
    TX_DEBUG_PRINT(("**********SIMG STARTING WAIT\n"));
	HalTimerWait(200);
	TX_DEBUG_PRINT(("**********SIMG FINISHED WAIT\n"));
    ...
    else
    {
    	if (gMhlDevice.pI2cClient == NULL)
        {
        	...
        	SII_DEBUG_PRINT(SII_OSAL_DEBUG_TRACE, "I2C driver add failed\n");
        	TX_DEBUG_PRINT(("I2C driver add failed 2\n"));
