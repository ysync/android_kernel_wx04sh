	...
	u16 reg, u8 *buf, int length)
{
	...
	memset(ts->wr_buf, 0, CY_SPI_DATA_SIZE + CY_SPI_WR_HEADER_BYTES);
	memset(ts->rd_buf, 0, CY_SPI_DATA_SIZE + CY_SPI_RD_HEADER_BYTES);
	...
	case CY_SPI_WR_OP:
		if ((length + CY_SPI_WR_HEADER_BYTES) > CY_SPI_DATA_SIZE) {
			dev_vdbg(ts->bus_ops.dev,
				...
				__func__, length, CY_SPI_WR_HEADER_BYTES,
				CY_SPI_DATA_SIZE);
			retval = -EINVAL;
	...
	case CY_SPI_RD_OP:
		...
		if ((length + CY_SPI_RD_HEADER_BYTES) > CY_SPI_DATA_SIZE) {
			dev_vdbg(ts->bus_ops.dev,
				"%s: length+%d=%d is greater than SPI max=%d.\n",
				__func__, length, CY_SPI_RD_HEADER_BYTES,
				CY_SPI_DATA_SIZE);
			retval = -EINVAL;
			goto cyttsp4_spi_xfer_exit;
		}
...
static int __devinit cyttsp4_spi_probe(struct spi_device *spi)
{
	...
	if (ts->pr_buf == NULL) {
		pr_err("%s: Error, kzalloc ts->pr_buf\n", __func__);
		kfree(ts->wr_buf);
		ts->wr_buf = NULL;
		kfree(ts->rd_buf);
		ts->rd_buf = NULL;
