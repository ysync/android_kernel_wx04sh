...
#undef	SHTPS_SMEM_BASELINE_ENABLE
...
struct shtps_rmi_spi {
	...
	struct work_struct			rezero_delayed_work;
		...
		struct workqueue_struct		*async_open_workqueue_p;
		struct work_struct			async_open_work_data;
...
static void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	if(fingerMax > 8){
		offset = 5;
		readsize = 5 + 5 * SHTPS_SY3X00_FINGER_SPIREAD_CNT;
	}else if (fingerMax > 4){
		offset = 4;
		readsize = 4 + 5 * SHTPS_SY3X00_FINGER_SPIREAD_CNT;
	}else{
		offset = 3;
	...
	memset(buf, 0, sizeof(buf));
		...
		shtps_rmi_read(ts, ts->map.fn01.dataBase, buf, 4);
			...
			shtps_rmi_read_block(ts, ts->map.fn01.dataBase, buf, readsize, ts->purposebuf_p);
		#else
...
#if defined( SHTPS_ASYNC_OPEN_ENABLE )
static void shtps_async_open( struct shtps_rmi_spi *ts )
{
	queue_work(ts->async_open_workqueue_p, &(ts->async_open_work_data) );
}

static void shtps_func_async_open_workq( struct work_struct *work_p )
{
	struct shtps_rmi_spi	*ts;

	ts = container_of(work_p, struct shtps_rmi_spi, async_open_work_data);
...
#if defined( SHTPS_ASYNC_OPEN_ENABLE )
	INIT_WORK( &(ts->async_open_work_data), shtps_func_async_open_workq );
	ts->async_open_workqueue_p = create_singlethread_workqueue( "TPS_OPEN" );
#endif /* #if defined( SHTPS_ASYNC_OPEN_ENABLE ) */
