...
static int inv_check_compass_self_test(struct inv_gyro_state_s *st, int *compass_result)
/* shmds mod <- */
{
	int result;
		...
		int *gyro_bias_regular)
{
	...
	if (st->chip_type == INV_ITG3500) {
		...
		if (st->has_compass)
			compass_result = !inv_check_compass_self_test(st, NULL);
...
/* shmds add -> */
int compass_hw_self_test(struct inv_gyro_state_s *st,
		int *compass_bias_regular)
{
	return inv_check_compass_self_test( st, compass_bias_regular );
}
/* shmds add <- */
/**
...
{
	const unsigned char regs_on[] = {DINAF8 + 1, DINA20,
					DINA28, DINA30, DINA38};
	const unsigned char regs_off[] = {DINAF8 + 1, 0xa3, 0xa3, 0xa3, 0xa3};
	...
	if (on)
		...
		regs = regs_off;
	result = mem_w_key(KEY_CFG_8, ARRAY_SIZE(regs_on), regs);
	return result;
}
