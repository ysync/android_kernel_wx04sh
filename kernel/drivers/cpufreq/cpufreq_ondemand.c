...
#define THRESHOLD_FREQ_RESPONSIVENESS	(702000)
#define THRESHOLD_FREQ_STEP				(1026000)
#define THRESHOLD_FREQ_INPUT_EVENT		(1026000)
#define SHSYS_CPU_DOWN_LIMIT			(918000)
#endif /* CONFIG_SHSYS_CUST */
...
#ifdef CONFIG_SHSYS_CUST
		int target;
		if (policy->cur < THRESHOLD_FREQ_RESPONSIVENESS)
			target = THRESHOLD_FREQ_STEP;
		else {
...
#ifdef CONFIG_SHSYS_CUST
	if (policy->cur < THRESHOLD_FREQ_INPUT_EVENT) {
		policy->cur = THRESHOLD_FREQ_INPUT_EVENT;

		__cpufreq_driver_target(policy, THRESHOLD_FREQ_INPUT_EVENT,
					CPUFREQ_RELATION_L);
...
#ifdef CONFIG_SHSYS_CUST
static void cpufreq_dbs_early_suspend(struct early_suspend *h)
{
	dbs_tuners_ins.freq_step = DEF_FREQ_STEP * 2 / 3;
}
static void cpufreq_dbs_late_resume(struct early_suspend *h)
{
	dbs_tuners_ins.freq_step = DEF_FREQ_STEP;
}
struct early_suspend dbs_early_suspend = {
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB,
	.suspend = cpufreq_dbs_early_suspend,
	.resume = cpufreq_dbs_late_resume,
};
#endif /* CONFIG_SHSYS_CUST */
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_STOP:
		...
		dbs_enable--;
#ifdef CONFIG_SHSYS_CUST
		if (cpu > 0 && this_dbs_info->cur_policy->cur > SHSYS_CPU_DOWN_LIMIT)
			__cpufreq_driver_target(this_dbs_info->cur_policy, SHSYS_CPU_DOWN_LIMIT, CPUFREQ_RELATION_L);
#endif /* CONFIG_SHSYS_CUST */
