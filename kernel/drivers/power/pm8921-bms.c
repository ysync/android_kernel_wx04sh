...
int pm8921_bms_get_vsense_for_rbatt(uint *result, uint *result_mA)
{
	...
	if (the_chip)
	{
		...
		if (!rc)
		{
			*result_mA = raw.last_good_ocv_uv / (int)the_chip->r_sense;
			pr_debug("vsense_for_milliamperes = %u\n", *result_mA);
		}
		return rc;
	}
...
int pm8921_get_bms_control(int *val)
{
	int rc;
	u8  reg;

	if (!val) {
		pr_err("val pointer null\n");
		return -EINVAL;
	}
	*val = 0;

	rc = pm8xxx_readb(the_chip->dev->parent, BMS_CONTROL, &reg);
	if (rc) {
		pr_err("fail to read BMS_CONTROL rc = %d\n", rc);
		return rc;
	}
	pr_err("BMS_CONTROL = %x\n", reg);
	*val = reg;
	
	return 0;
}
EXPORT_SYMBOL(pm8921_get_bms_control);
#endif /* CONFIG_BATTERY_SH */
