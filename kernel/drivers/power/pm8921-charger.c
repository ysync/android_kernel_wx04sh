...
#if defined(CONFIG_CRADLE_SWIC) && !defined(CONFIG_CRADLE_DCIN_SH)
#define __SUPPORT_CRADLE__
#endif /* CONFIG_CRADLE_SWIC && Not CONFIG_CRADLE_DCIN_SH */
...
#ifdef CONFIG_BATTERY_SH
#define SH_CHG_CURRENT_THRESHOLD	-10
#define SH_CHG_VOLTAGE_THRESHOLD	3400
#endif /* CONFIG_BATTERY_SH */
...
static void __pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	if (sh_boot_mode != SHBATT_BOOT_MODE_FUNC_TEST) {
		if ((mA == 0) || (mA == 2)) {
			pr_info("ous: mA = %u status = %d\n", mA, sh_chg_status);
			if (sh_chg_status > 0) {
				sh_chg_status = 0;
			}
		} else
		if ((mA == 500) || (mA == 1500)) {
			pr_info("ins: mA = %u status = %d\n", mA, sh_chg_status);
			if (sh_chg_status < 1) {
				if (get_prop_batt_current(the_chip) < SH_CHG_CURRENT_THRESHOLD) {
					sh_chg_status = 1;
				} else {
					sh_chg_status = 0;
				}
...
/* COORDINATOR Qualcomm1740J MERGE end */
		...
		if (rc == 0)
			return CHG_IN_PROGRESS;
#else
/* COORDINATOR Qualcomm1740J BUILDERR MODIFY start */
//		vbatdet_low = 1;
/* COORDINATOR Qualcomm1740J BUILDERR MODIFY end */
#endif /* CONFIG_SHSYS_CUST */
...
static int __devinit pm8921_chg_hw_init(struct pm8921_chg_chip *chip)
{
	int rc;
	int vdd_safe;
/* COORDINATOR Qualcomm17382J BUILDERR MODIFY start */
//	u8 subrev;
/* COORDINATOR Qualcomm17382J BUILDERR MODIFY end */
...
/* COORDINATOR Qualcomm17382J MERGE start */
	/* Workarounds for SMBC PFET 100% duty cycle issue */
	{
		u8 temp1;
		int err1;
		...
		err1 = pm8xxx_readb(chip->dev->parent, PM_SUB_REV, &temp1);
		temp1 &= 0x1;
	
		if (pm8xxx_get_revision(chip->dev->parent) == PM8XXX_REVISION_8921_3p0) {
			if(temp1==1)
	    	    pm8xxx_writeb(chip->dev->parent, CHG_BUCK_CTRL_TEST3, 0xA4);
		    else
