...
#ifdef CONFIG_BATTERY_SH
	if (chan_prop->amux_channel == CHANNEL_MUXOFF)
		dbg_flag = 1;
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
	if (dbg_flag) {
		rc = pm8xxx_adc_read_reg(PM8XXX_ADC_ARB_USRP_ANA_PARAM,
							&data_ana_dbg);
		data_ana_dbg |= PM8XXX_ADC_ARB_USRP_ANA_PARAM_VREF_XO_THM;
		/* Force VREF_XO_THM on during sleep state. This is
		 * for debug.
		 */
		rc = pm8xxx_adc_write_reg(PM8XXX_ADC_ARB_USRP_ANA_PARAM,
						data_ana_dbg);
