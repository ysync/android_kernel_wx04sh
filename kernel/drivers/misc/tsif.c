 ...
 *
 */
#if defined( CONFIG_DTV_TUNERDEV_TMM )
//#define TSIF_PKTS_IN_CHUNK_DEFAULT  (32)  /**< packets in one DM chunk */
...
tsif_2nd_buffer_t *p2nd_buffer;

static tsif_2nd_buffer_t  *tsif_2nd_buffer_create(void);
static void  tsif_2nd_buffer_destroy(tsif_2nd_buffer_t *pbuffer);
#endif	// CONFIG_DTV_TUNERDEV_TMM
...
{
	if( (num >= p2nd_buffer->cnt_data_buffer_ptr * TSIF_NUM_OF_EACH_TSBUFFER) || (num < 0)){
		return NULL;
	}
	return  &( ( p2nd_buffer->data_buffer_ptr[num/TSIF_NUM_OF_EACH_TSBUFFER] ) [ (num % TSIF_NUM_OF_EACH_TSBUFFER) * TSIF_PKT_SIZE ] );
}
#endif
...
#if !(defined( CONFIG_DTV_TUNERDEV ) || defined( CONFIG_DTV_TUNERDEV_TMM ))
				    ...
				    struct msm_dmov_errdata *err)
{
	...
	if (result & DMOV_RSLT_DONE) {
		...
		if(reschedule) {
			...
			memcpy( p2nd_buffer->data_buffer_ptr[p2nd_buffer->wi / TSIF_PKTS_IN_CHUNK], tsif_device->data_buffer + p2nd_buffer->dev_rptr, TS_XFER_SIZE);

			p2nd_buffer->wi = (p2nd_buffer->wi + TSIF_PKTS_IN_CHUNK) % (TSIF_NUM_OF_EACH_TSBUFFER * p2nd_buffer->cnt_data_buffer_ptr);
...
static tsif_2nd_buffer_t *tsif_2nd_buffer_create(void)
{
    tsif_2nd_buffer_t* pbuffer = NULL;
	int i;


    pbuffer = (tsif_2nd_buffer_t *)kzalloc(sizeof(tsif_2nd_buffer_t), GFP_KERNEL);
    
    if (NULL != pbuffer) {
    	pbuffer->cnt_data_buffer_ptr = 0;
		for(i=0; i<TSIF_MM_TSBUFFER_WIDTH; i++){
 
	        pbuffer -> data_buffer_ptr[i] = (unsigned char *)kzalloc(TSIF_PKT_SIZE*TSIF_NUM_OF_EACH_TSBUFFER, GFP_KERNEL);
	        if (NULL == pbuffer -> data_buffer_ptr[i]) 
			{
				printk("TSIF:kmalloc failure[data_buffer_ptr[%d]]\n", i);
				break;
	            //kfree((void *)pbuffer);
	            //kfree(pbuffer);
	            //return NULL;
	    ...
	    pbuffer->cnt_data_buffer_ptr = i;        
    	if(pbuffer -> cnt_data_buffer_ptr < TSIF_MM_TSBUFFER_WIDTH / 2){
			printk("TSIF: allocated TSbuffer is insufficient (%d < %d) \n", pbuffer -> cnt_data_buffer_ptr, TSIF_MM_TSBUFFER_WIDTH/2);	
			for(i=0; i<pbuffer -> cnt_data_buffer_ptr;i++){
				kfree(pbuffer -> data_buffer_ptr[i]);
			}
			pbuffer -> cnt_data_buffer_ptr = 0;
			kfree(pbuffer);
			return NULL;
		...
		printk("TSIF:kmalloc failure[fifo]\n");
	}
...
#if defined( CONFIG_DTV_TUNERDEV_TMM )
	p2nd_buffer = tsif_2nd_buffer_create();

    if (NULL == p2nd_buffer) {
		printk("TSIF:2nd buffer alloc error\n");
		return -EINVAL;
	}
#endif	// CONFIG_DTV_TUNERDEV_TMM
...
#if defined( CONFIG_DTV_TUNERDEV_TMM )
	tsif_2nd_buffer_destroy(p2nd_buffer);
#endif	// CONFIG_DTV_TUNERDEV_TMM
