...
#else  /* CONFIG_SH_AUDIO_DRIVER *//*02-069*/
		if (rc < 0)
			return rc;
#endif /* CONFIG_SH_AUDIO_DRIVER *//*02-069*/
		vib->reg_vib_drv = val;
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-093*/
/* vibrator moving time change    */
	if (value <= 0) /* Check Signed Value */
		vib->state = 0;
	else {
		if( p_smem_addr == NULL) {
			printk( KERN_ERR "[pm8xxx_vib_enable] p_sharp_smem_common_type == NULL!!\n" );
			value = (value > vib->pdata->max_timeout_ms ?
					 vib->pdata->max_timeout_ms : value);
			printk( KERN_ERR "[pm8xxx_vib_enable] vib timeout_ms = %d ms\n", value);
		}
		else {
			if(0xFFFF == (p_smem_addr->sh_boot_mode)) { /* Check SH_BOOT_NORMAL */
				value = (value > vib->pdata->max_timeout_ms ?
						 vib->pdata->max_timeout_ms : value);
			}
		}
