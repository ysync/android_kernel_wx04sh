...
#define VDEC_PM_QOS_VALUE		(1299)
#define VDEC_SECURE_PM_QOS_VALUE	(1699)
...
static int vid_dec_open_secure(struct inode *inode, struct file *file)
{
	...
	if (client_ctx) {
		client_ctx->qos_cpu_dma_latency.pm_qos_class = PM_QOS_RESERVED;
		pm_qos_add_request(&client_ctx->qos_cpu_dma_latency,PM_QOS_CPU_DMA_LATENCY,VDEC_SECURE_PM_QOS_VALUE);
		INFO("msm_vidc_dec: %s(VDEC_SECURE_PM_QOS_VALUE=%d) qos_add_request", __func__,VDEC_SECURE_PM_QOS_VALUE);
	} else {
		INFO("msm_vidc_dec: %s() no qos_add_request", __func__);
	}
...
static int vid_dec_open(struct inode *inode, struct file *file)
{
	...
	if (client_ctx) {
		client_ctx->qos_cpu_dma_latency.pm_qos_class = PM_QOS_RESERVED;
		pm_qos_add_request(&client_ctx->qos_cpu_dma_latency,PM_QOS_CPU_DMA_LATENCY,VDEC_PM_QOS_VALUE);
		INFO("msm_vidc_dec: %s(VDEC_PM_QOS_VALUE=%d) qos_add_request", __func__,VDEC_PM_QOS_VALUE);
	} else {
		INFO("msm_vidc_dec: %s() no qos_add_request", __func__);
	}
...
static int vid_dec_release_secure(struct inode *inode, struct file *file)
{
	...
	if (client_ctx) {
		if (client_ctx->qos_cpu_dma_latency.pm_qos_class == PM_QOS_CPU_DMA_LATENCY) {
			pm_qos_remove_request(&client_ctx->qos_cpu_dma_latency);
			INFO("msm_vidc_dec: %s() qos_remove_request", __func__);
		} else {
			INFO("msm_vidc_dec: %s() no qos_remove_request", __func__);
		}
	}
