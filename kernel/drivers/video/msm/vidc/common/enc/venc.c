...
static int vid_enc_open(struct inode *inode, struct file *file)
{
	...
	if (client_ctx) {
		...
		pm_qos_add_request(&client_ctx->qos_cpu_dma_latency,PM_QOS_CPU_DMA_LATENCY,VENC_PM_QOS_VALUE);
		INFO("msm_vidc_enc: %s(VENC_PM_QOS_VALUE=%d) qos_add_request", __func__,VENC_PM_QOS_VALUE);
	} else {
		INFO("msm_vidc_enc: %s() no qos_add_request", __func__);
	}
...
static int vid_enc_release(struct inode *inode, struct file *file)
{
	...
	if (client_ctx) {
		if (client_ctx->qos_cpu_dma_latency.pm_qos_class == PM_QOS_CPU_DMA_LATENCY) {
			...
			INFO("msm_vidc_enc: %s() qos_remove_request", __func__);
		} else {
			INFO("msm_vidc_enc: %s() no qos_remove_request", __func__);
		}
	}
