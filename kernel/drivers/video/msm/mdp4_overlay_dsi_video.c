...
static void mdp4_video_vsync_ctrl_db_out(void)
{
	struct vsycn_ctrl *vctrl;
	vctrl = &vsync_ctrl_db[0];

	pr_err("[ vsycn_ctrl ] inited=%d,update_ndx=%d,ov_koff=%lu,ov_done=%lu\n", 
			vctrl->inited,
			vctrl->update_ndx,
			(unsigned long)vctrl->ov_koff,
			(unsigned long)vctrl->ov_done
	);
	pr_err("[ ---------- ] suspend=%d,vsync_resume=%d,wait_vsync_cnt=%d,blt_change=%d,blt_free=%d,sysfs_created=%d\n", 
	...
	pr_err("[ ---------- ] vsync_irq_enabled=%d,wait_show_cnt=%d\n", 
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00058 */
	if (overlay_stop) {
...
void mdp4_dsi_video_wait4vsync(int cndx, long long *vtime)
{
	...
	int_cnt = mdp4_stat.intr_vsync_p;
	...
	if (wait_ret <= 0) {
		...
		pr_err("dsi_clk=%d->%d, mdp_clk=%d->%d, isr=%d, count=%lu->%lu\n",
				dsi_clk_info, mipi_dsi_clk_on, mdp_clk_info, mdp_get_clk_cnt(), mdp_is_in_isr, int_cnt, mdp4_stat.intr_vsync_p);
		mdp_clk_info_out();
		mdp4_video_vsync_ctrl_db_out();
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00155 */
	mutex_unlock(&mfd->dma->ov_mutex);
#endif /* CUST_ID_00155 */
}
