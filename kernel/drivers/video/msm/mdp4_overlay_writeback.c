...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00095 */ /* CUST_ID_00096 */ /* CUST_ID_00098 */
...
int mdp4_overlay_writeback_off(struct platform_device *pdev)
{
	...
	mdp4_overlay_iommu_unmap_freelist(pipe->mixer_num);
#endif	/* CUST_ID_00141 */
...
#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
static void mdp4_wfd_wait4ov(int cndx, ulong addr);
#else	/* CUST_ID_00141 */
...
#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
	if(node == NULL){
		pr_debug("%s: NULL node\n", __func__);
		return cnt;
	}
#endif	/* CUST_ID_00141 */

#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
	mdp4_wfd_wait4ov(cndx, 0);
	wait=0;
	if (cancel_work_sync(&vctrl->clk_work) == 0){
		pr_debug("%s: cancel_work_sync()==0\n", __func__);
		mdp_clk_ctrl(1);
	}
#else	/* CUST_ID_00141 */
	mdp_clk_ctrl(1);
...
#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
	vctrl->blt_addr = pipe->ov_blt_addr;
#endif	/* CUST_ID_00141 */
	vctrl->ov_koff++;
...
#endif	/* CUST_ID_00141 */
{
	...
	if(addr != 0 && addr != vctrl->blt_addr){
		spin_unlock_irqrestore(&vctrl->spin_lock, flags);
		return;
	}
	spin_unlock_irqrestore(&vctrl->spin_lock, flags);
	pr_debug("%s: addr=0x%08lx, vctrl->blt_addr=0x%08lx\n", __func__, (unsigned long)addr, (unsigned long)vctrl->blt_addr);
#endif	/* CUST_ID_00141 */
...
#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
	if(node)
		mdp4_wfd_wait4ov(0, (ulong)node->addr);
#endif	/* CUST_ID_00141 */
	return rc;
}
