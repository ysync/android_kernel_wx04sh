...
{
#ifdef CONFIG_SII8334_MHL_TX
	const u8 *buf = edid_buf;
	char *shmhl_edid_buf = NULL;
	int print_size = 0;

	uint32 ndx, print_len;
#else
...
#ifdef DEBUG
*/
#if defined(SHMHL_DEBUG_LOG_SYS_FS_ENTRY) || defined(DEBUG)
/* SHMHL Mod End <-- */
	const u8 *b = edid_buf;
#endif
...
#ifdef CONFIG_SII8334_MHL_TX
	shmhl_edid_buf = shmhl_get_edid_out_buf(block);
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
		if(NULL != shmhl_edid_buf) {
			print_size = sprintf(shmhl_edid_buf,
				"EDID[%02x-%02x] %02x %02x %02x %02x  "
				"%02x %02x %02x %02x    %02x %02x %02x %02x  "
				"%02x %02x %02x %02x\n", ndx, ndx+15,
				buf[ndx+0], buf[ndx+1], buf[ndx+2], buf[ndx+3],
			...
			shmhl_edid_buf = shmhl_edid_buf + print_size;
		}
	}
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
		DEV_ERR("%s: Not support CEA block:%d\n", __func__, num_og_cea_blocks);
		external_common_state->hdmi_sink = false;
		status = -EPROTO;
		goto error;
#else
